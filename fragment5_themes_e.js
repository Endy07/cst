...
function initEventHandlers() {
    ...
    if (btnAlignTop) btnAlignTop.addEventListener('click', () => window.setElementAlign('top'));
    if (btnAlignCenter) btnAlignCenter.addEventListener('click', () => window.setElementAlign('center'));
    if (btnAlignBottom) btnAlignBottom.addEventListener('click', () => window.setElementAlign('bottom'));

    // Maszk Gombok
    const maskTypes = ['none', 'classic', 'elegant', 'round'];
    maskTypes.forEach(type => {
        const btn = document.getElementById(`c-mask-btn-${type}`);
        if (btn) btn.addEventListener('click', () => window.setCommonMask(type));
    });
...
window._refreshMapTransformImpl = function () {
    ...
    elements.forEach((el, index) => {
        ...
        // --- LOKÁLIS HÁTTÉR (JAVÍTVA: GRADIENS TÁMOGATÁSSAL) ---
...
window.applyDesignerTheme = function (key, variant = 'normal') {
    ...
    console.log(`DEBUG_APPLY: applyDesignerTheme called. key=${key}, variant=${variant}, detectedSide=${side}`);
...
window.updateCanvasBackground = function (color) {
    ...
    if (colorInput) {
        ...
        // Ha gradiens, próbáljuk kinyerni az első színt a picker számára
...
window.setBleedMode = function (mode) {
    console.log(`📐 setBleedMode: mode=${mode}`);
    if (!myCelestialConf.userData) initUserData();
    ...
    myCelestialConf.userData.canvas.bleed.mode = mode;

    const customPanel = document.getElementById('custom-bleed-panel');
    if (customPanel) {
        customPanel.style.display = (mode === 'custom') ? 'block' : 'none';
    }

    // Ha élőnézet aktív, frissítsük
    if (myCelestialConf.userData.canvas.bleed.previewEnabled) {
        window.updateBleedPreview();
    }
};

// Egyéni kifutó szín beállítása
window.updateCustomBleedColor = function (color) {
    console.log(`📐 updateCustomBleedColor: color=${color}`);
    if (!myCelestialConf.userData) initUserData();
    if (!myCelestialConf.userData.canvas.bleed) {
        myCelestialConf.userData.canvas.bleed = { mode: 'custom', customColor: '#000000', previewEnabled: false };
    }

    myCelestialConf.userData.canvas.bleed.customColor = color;
    currentBleedGradientState.isGradient = false;
...
window.addBleedGradientStop = function (colorValue = '#ffffff', percentValue = null) {
    ...
    row.innerHTML = `
        <span class="grad-drag-handle" style="cursor:grab; color:#888; font-size:14px; padding-left:5px; padding-right:5px;">☰</span>
        <input type="color" class="grad-stop-input" value="${colorValue}" oninput="window.updateBleedGradientFromEditor()">
        <input type="number" class="grad-stop-percent" value="${percentValue}" min="0" max="100" step="1"
               style="width: 45px; height: 32px; background: rgba(0,0,0,0.3); border: 1px solid #444; border-radius: 4px; color: white; text-align: center; font-size: 11px;"
               oninput="window.updateBleedGradientFromEditor()">
        <button class="grad-remove-btn" title="Törlés" onclick="this.parentElement.remove(); window.updateBleedGradientFromEditor();">✕</button>
    `;
    list.appendChild(row);
    window.updateBleedGradientFromEditor();
};
...
window.updateBleedPreview = function () {
    console.log(`📐 updateBleedPreview called (new approach)`);

    const wrapper = document.getElementById('canvas-wrapper');
    const bleedBg = document.getElementById('bleed-preview-bg');

    if (!wrapper || !bleedBg) {
        console.warn(`📐 Missing elements: wrapper=${!!wrapper}, bleedBg=${!!bleedBg}`);
        return;
    }
    ...
    const bleedPxWidth = 2 * pxPerCmWidth;
    const bleedPxHeight = 2 * pxPerCmHeight;

    console.log(`📐 Bleed preview: totalSize=${totalWidthCm}x${totalHeightCm}cm, wrapper=${wrapperWidthPx.toFixed(1)}x${wrapperHeightPx.toFixed(1)}px`);
    console.log(`📐 Bleed preview: bleedPx=${bleedPxWidth.toFixed(1)}x${bleedPxHeight.toFixed(1)}px, mode=${bleedSettings.mode}`);

    // Determine bleed color
    let bleedColor = '#000000';
    if (bleedSettings.mode === 'custom') {
        bleedColor = bleedSettings.customColor;
    } else {
        // Theme mode: canvas background
        bleedColor = myCelestialConf.userData.canvas.background || '#0a0e27';
    }

    // Calculate bleed background size and position
    // The bleed bg should be centered behind the wrapper, but larger by bleedPx on all sides
    const bleedBgWidth = wrapperWidthPx + (2 * bleedPxWidth);
    const bleedBgHeight = wrapperHeightPx + (2 * bleedPxHeight);

    // Get wrapper position relative to parent (designer-canvas-area)
    const parent = wrapper.parentElement;
    const parentRect = parent.getBoundingClientRect();
    const wrapperLeft = wrapperRect.left - parentRect.left;
    const wrapperTop = wrapperRect.top - parentRect.top;

    // Position bleed bg centered behind wrapper
    const bleedBgLeft = wrapperLeft - bleedPxWidth;
    const bleedBgTop = wrapperTop - bleedPxHeight;

    // Apply styles to bleed background
    bleedBg.style.display = 'block';
    bleedBg.style.width = `${bleedBgWidth}px`;
    bleedBg.style.height = `${bleedBgHeight}px`;
    bleedBg.style.left = `${bleedBgLeft}px`;
    bleedBg.style.top = `${bleedBgTop}px`;
    bleedBg.style.background = bleedColor;

    console.log(`📐 Bleed preview: bgSize=${bleedBgWidth.toFixed(1)}x${bleedBgHeight.toFixed(1)}px, pos=${bleedBgLeft.toFixed(1)},${bleedBgTop.toFixed(1)}`);
    console.log(`📐 Bleed preview added with color: ${bleedColor}`);
};
...
window.updateElementSelectorUI = function () {
    ...
    // --- 4. HELY: NÉZET MENÜ ELEM VÁLASZTÓ (PREVIEW) ---
    const previewContainer = document.getElementById('preview-element-selector-container');
    const previewSelect = document.getElementById('active-element-selector-preview');

    if (previewContainer && previewSelect) {
        if (allElements.length > 1) {
            previewContainer.style.display = 'block';
            previewSelect.innerHTML = buildOptionsHTML(false); // Nincs közös opció

            // Aktuális érték beállítása
            let previewVal = currentCtx;
            if (currentCtx === 'map') previewVal = 'map_main-map';
            else if (currentCtx === 'common') {
                const selId = uiState.selectedElementId;
                const el = allElements.find(e => e.id == selId);
                if (el) previewVal = (el.type === 'map') ? `map_${el.id}` : `photo_${el.id}`;
            }

            if (previewVal && previewSelect.querySelector(`option[value="${previewVal}"]`)) {
                previewSelect.value = previewVal;
            }
        } else {
    ...
    // --- 5. ALKALMAZÁSI MÓDOK LÁTHATÓSÁGÁNAK FRISSÍTÉSE ---
...
window.handleTemplateElementSelection = function (value) {
    if (typeof window.logDebug === 'function') window.logDebug(`DEBUG_SELECT: value=${value}`);
    if (!value) return;

    if (value === 'map' || value === 'common') {
        // Fallback/Legacy handling: assume main-map
        value = 'map_main-map';
    }
    ...
    if (el) {
        ...
        const otherSelectors = ['active-element-selector', 'text-mode-selector', 'active-element-selector-preview'];
...
window.openFullScreenView = function () {
    const designerSvg = document.getElementById('designer-svg');
    if (!designerSvg) {
        if (window.showCustomAlert) window.showCustomAlert("Nem található a tervező vászon!", 'error');
        return;
    }
    ...
    // 1. Tartalom klónozása
    container.innerHTML = '';
    const clone = designerSvg.cloneNode(true);
    clone.id = 'fs-preview-svg';

    // BUGFIX: First remove old styles, THEN set new ones.
    clone.removeAttribute('style'); // Remove original styles (width/height pixels)
    clone.style.width = '100%';
    clone.style.height = '100%';
    clone.style.maxWidth = 'none';
    clone.style.maxHeight = 'none';
    clone.style.display = 'block'; // Ensure it's not hidden
    clone.style.background = 'rgb(10, 14, 39)'; // Ensure dark background

    // CANVAS INJECTION FIX:
    // Mivel a térkép Canvas-on van, a cloneNode nem hozza át.
    // Megkeressük az eredeti canvast és képként beszúrjuk a klónba.
    const originalCanvasContainer = document.getElementById('celestial-map');
    let canvas = originalCanvasContainer ? originalCanvasContainer.querySelector('canvas') : null;

    // Ha ott nincs, keressük bárhol
    if (!canvas) canvas = document.querySelector('canvas');

    console.log("[FS Debug] Canvas detection:", !!canvas, canvas ? { w: canvas.width, h: canvas.height } : "N/A");

    if (canvas) {
        try {
            const dataUrl = canvas.toDataURL("image/png");
            console.log("[FS Debug] DataURL generated. Length:", dataUrl ? dataUrl.length : 0);

            const svgImage = document.createElementNS("http://www.w3.org/2000/svg", "image");


            // Méretek és pozíció:
            // Feltételezzük, hogy a canvas kitölti a teljes ViewBox-ot (vagy a papírt).
            // A legbiztosabb, ha a ViewBox méreteit vesszük alapul.
            const vb = designerSvg.getAttribute('viewBox').split(' ').map(Number);
            const vbW = vb[2];
            const vbH = vb[3];

            svgImage.setAttribute('x', vb[0] || 0);
            svgImage.setAttribute('y', vb[1] || 0);
            svgImage.setAttribute('width', vbW);
            svgImage.setAttribute('height', vbH);
            svgImage.setAttributeNS("http://www.w3.org/1999/xlink", "href", dataUrl);
            svgImage.setAttribute("href", dataUrl); // Fallback for modern browsers
            svgImage.style.pointerEvents = "none"; // Non-interactive background

            // CLIPPING/OCCLUSION FIX:
            // Insert at the END (appendChild) to force it on top of potentially opaque background layers.
            // We set pointer-events: none so it doesn't block interactions.
            clone.appendChild(svgImage);

            // Debugging: ensure it's visible
            console.log("Injected map image at the TOP of stack", svgImage);
        } catch (e) {
            console.error("Canvas snapshot failed:", e);
        }
    }

    // Keep background color from original (Robust check)
    // Ha van injektált kép (ami általában fekete/sötét alapú), akkor a háttérszín kevésbé kritikus,
    // de a biztonság kedvéért meghagyjuk.
    const compStyle = window.getComputedStyle(designerSvg);
    const bgColor = compStyle.backgroundColor;
    // Ha átlátszó, akkor alapértelmezett sötétkék (hogy látszódjon a sötét overlayen is, ha a vonalak világosak)
    // De ha világos téma van, akkor a világos háttér kell.
    if (!bgColor || bgColor === 'rgba(0, 0, 0, 0)' || bgColor === 'transparent') {
        clone.style.backgroundColor = myCelestialConf.userData.canvas.background || '#0a0e27';
    } else {
        clone.style.backgroundColor = bgColor;
    }

    // 2. View Mode kezelése
    if (window.fsViewMode === 'selected') {
        if (!myCelestialConf.userData) initUserData();
        const selId = myCelestialConf.userData.uiState.selectedElementId;

        if (selId) {
            // Megpróbáljuk beállítani a viewBox-ot a kiválasztott elemre
            const el = myCelestialConf.userData.elements.find(e => e.id == selId);
            if (el) {
                // Számoljunk BBox-ot a papír méretei alapján
                const layoutDir = myCelestialConf.userData.canvas.layoutDirection || 'row';
                const paperVB = designerSvg.getAttribute('viewBox').split(' ').map(Number);
                const paperW = paperVB[2];
                const paperH = paperVB[3];
                const count = myCelestialConf.userData.elements.length;
                const index = myCelestialConf.userData.elements.findIndex(e => e.id == selId);

                let slotW, slotH, slotX, slotY;
                if (layoutDir === 'column') {
                    slotW = paperW; slotH = paperH / count;
                    slotX = 0; slotY = index * slotH;
                } else {
                    slotW = paperW / count; slotH = paperH;
                    slotX = index * slotW; slotY = 0;
                }

                // Beállítjuk a klón viewBox-át erre a területre
                clone.setAttribute('viewBox', `${slotX} ${slotY} ${slotW} ${slotH}`);
            }
        }
    }
    ...
    // Eseményfigyelők (globális, hogy ne ragadjon be a drag)
...
window.setPreviewZoom = function (val) {
    ...
    if (val === 'fit') {
        window.fsCurrentZoom = 0.9; // Kis margó
        window.fsPanX = 0;
        window.fsPanY = 0;
        // Reset transform
    ...
    if (val === 1) { // 1:1 Valós méret (kb)
        ...
        const targetPx = widthCm * 37.8;
        const rect = container.getBoundingClientRect();
        const currentUnscaledWidth = rect.width / window.fsCurrentZoom;

        if (currentUnscaledWidth > 0) {
            let newScale = targetPx / currentUnscaledWidth;
