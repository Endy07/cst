...
//         // --- ÚJ: LOKÁLIS HÁTTÉR RENDERELÉSE ---
//         if (el.localBackground) {
//             const bgRect = document.createElementNS("http://www.w3.org/2000/svg", "rect");
//             // A rect mérete megegyezik a tartalom alapméretével (1000x1000)
//             bgRect.setAttribute('width', contentBaseSize);
//             bgRect.setAttribute('height', contentBaseSize);
//             bgRect.setAttribute('x', 0);
//             bgRect.setAttribute('y', 0);
            
//             // Ha maszk van, a hátteret is maszkolni kell, különben kilóg "négyzetesen"
//             // De ezt a legegyszerűbb úgy, ha a háttér a contentNode mögé kerül, 
//             // és ugyanazt a clip-path-ot kapja meg (lásd lejjebb).
            
//             // Kezeljük a gradienseket
//             if (el.localBackground.includes('gradient')) {
//                 // Egyszerűsítés: Ha gradiens, akkor (egyelőre) vegyük az első színt, 
//                 // vagy definiáljunk egy ID-t a defs-ben.
//                 // A legegyszerűbb megoldás itt a CSS background trükk helyett 
//                 // egy szolid szín kinyerése, mert SVG-ben a CSS gradiens nem mindig megy a fill-ben.
//                 // VAGY: Egyedi ID generálás.
                
//                 // Most fallback a biztonság kedvéért: Fekete vagy az első szín
//                 const match = el.localBackground.match(/#[a-fA-F0-9]{6}/);
//                 bgRect.setAttribute('fill', match ? match[0] : '#000000');
//             } else {
//                 bgRect.setAttribute('fill', el.localBackground);
//             }
            
//             g.appendChild(bgRect); // Hozzáadjuk a csoporthoz (a tartalom alá)
//         }
//         // --------------------------------------
...
//                 // Háttér maszkolása (ha van lokális háttér)
//                 const bgRect = g.querySelector('rect[width="1000"]'); // Megkeressük a fenti rect-et
//                 if (bgRect) bgRect.setAttribute('mask', `url(#${maskId})`);
...
//                 // Háttér maszkolása (ha van lokális háttér)
//                 const bgRect = g.querySelector('rect[width="1000"]');
//                 if (bgRect) bgRect.setAttribute('clip-path', `url(#${clipId})`);
//             }
//         }
...
//             defs.appendChild(filter);
//         }
//         filter.querySelector("feGaussianBlur").setAttribute("stdDeviation", blurAmount);
//         return (blurAmount > 0) ? `url(#${filterId})` : null;
//     };

//     elements.forEach((el, index) => {
//         // 1. Pozíció számítása (Slot)
//         let slotStartX, slotStartY, slotCenterX, slotCenterY;
//         if (layoutDir === 'column') {
//             slotStartX = 0; slotStartY = index * slotH;
//             slotCenterX = paperW / 2; slotCenterY = slotStartY + (slotH / 2);
...
//         // 2. Csoport létrehozása és transzformálása
//         const g = document.createElementNS("http://www.w3.org/2000/svg", "g");
//         g.setAttribute('id', `element-group-${el.id}`);
//         g.setAttribute('class', `designer-element type-${el.type}`);
...
//         // Eltolás a slot közepéhez képest
//         let transX = slotCenterX - (targetSizePx / 2);
//         let transY = 0;
...
//         // ITT TÖRTÉNIK AZ ELTOLÁS! Ezért belül a 0,0 a helyes.
//         g.setAttribute('transform', `translate(${transX}, ${transY}) rotate(${rot}, ${rotCenter}, ${rotCenter}) scale(${finalScale})`);

//         // --- 3. LOKÁLIS HÁTTÉR (JAVÍTOTT: Változóba mentjük!) ---
//         let localBgRect = null;
//         if (el.localBackground) {
//             localBgRect = document.createElementNS("http://www.w3.org/2000/svg", "rect");
//             localBgRect.setAttribute('width', contentBaseSize);
//             localBgRect.setAttribute('height', contentBaseSize);
//             localBgRect.setAttribute('x', 0); // Relatív 0
//             localBgRect.setAttribute('y', 0); // Relatív 0
            
//             if (el.localBackground.includes('gradient')) {
//                 // Egyszerűsített megoldás: gradiens első színe
//                 const match = el.localBackground.match(/#[a-fA-F0-9]{6}/);
//                 localBgRect.setAttribute('fill', match ? match[0] : '#000000');
//             } else {
//                 localBgRect.setAttribute('fill', el.localBackground);
//             }
//             g.appendChild(localBgRect);
//         }

//         // --- 4. TARTALOM (Térkép / Kép) ---
//         let contentNode = null;
...
//             const svgNode = tempDiv.querySelector('svg');
//             if (svgNode) {
//                 // Kényszerített méret a csoporton belül
//                 svgNode.setAttribute('width', contentBaseSize);
//                 svgNode.setAttribute('height', contentBaseSize);
//                 svgNode.setAttribute('x', 0);
//                 svgNode.setAttribute('y', 0);
//                 svgNode.setAttribute('preserveAspectRatio', 'xMidYMid meet'); // Fontos!
//                 svgNode.style.pointerEvents = "none"; 
//                 contentNode = svgNode;
//             }
//         }
...
//         // --- 5. MASZKOLÁS (Képre ÉS Háttérre is!) ---
//         const maskType = el.mask || 'none';
//         const maskScale = el.maskScale || 1.0;
//         const edgeBlur = el.edgeBlur || 0;
//         const maskId = `mask-${el.id}`;
        
//         // Régi maszkok törlése
//         const oldMask = defs.querySelector(`#${maskId}`); if(oldMask) oldMask.remove();

//         let needsMasking = (maskType !== 'none');
//         if (el.type === 'photo' && maskType === 'none') needsMasking = true;

//         if (needsMasking) {
//             // A) Alakzat létrehozása
//             const createShape = () => {
//                 if (typeof HEART_PATHS !== 'undefined' && HEART_PATHS[maskType]) {
//                     const p = document.createElementNS("http://www.w3.org/2000/svg", "path");
//                     p.setAttribute('d', HEART_PATHS[maskType]);
...
//             // B) Mask element létrehozása a defs-be
//             const maskEl = document.createElementNS("http://www.w3.org/2000/svg", "mask");
//             maskEl.setAttribute('id', maskId);
//             const shape = createShape();
//             shape.setAttribute('fill', 'white'); // Ami fehér, az látszik
...
//             // C) Maszk alkalmazása a TARTALOMRA
//             // Ha SVG a tartalom, be kell csomagolni egy csoportba a maszkoláshoz
//             if (contentNode.tagName === 'svg' || contentNode.tagName === 'g') {
//                 const wrapperG = document.createElementNS("http://www.w3.org/2000/svg", "g");
//                 wrapperG.setAttribute('mask', `url(#${maskId})`);
//                 wrapperG.appendChild(contentNode);
//                 contentNode = wrapperG;
...
//             // D) Maszk alkalmazása a LOKÁLIS HÁTTÉRRE (ITT VOLT A HIBA)
//             // Most közvetlenül a változóra hivatkozunk, nem querySelectorral keressük!
//             if (localBgRect) {
//                 localBgRect.setAttribute('mask', `url(#${maskId})`);
//             }
//         }

//         g.appendChild(contentNode); // Hozzáadjuk a tartalmat (a háttér fölé)
...
// };
// // ============================================================
// // --- JAVÍTOTT: refreshMapTransform (Renderelő és Pozicionáló) ---
// // ============================================================
// window.refreshMapTransform = function() {
//     if (!getDOMElements()) return;
//     if (!myCelestialConf.userData) initUserData();
...
//     // Pixel/CM arány számítása
//     const canvasWInput = document.getElementById('canvas-width');
//     const pageWidthCm = parseFloat(canvasWInput ? canvasWInput.value : 21);
//     // Ha sorban vannak, a teljes szélesség a lapok összege
//     let effectivePageWidthCm = (layoutDir === 'row' && elements.length > 1) ? pageWidthCm * elements.length : pageWidthCm;
//     const pxPerCm = paperW / effectivePageWidthCm;

//     // Slot méretek (Helyek felosztása)
//     const count = elements.length;
//     let slotW, slotH;
//     if (layoutDir === 'column') {
//         slotW = paperW; slotH = paperH / count;
...
//     // --- ELEMEK RENDERELÉSE ---
//     elements.forEach((el, index) => {
//         // 1. POZÍCIÓ SZÁMÍTÁSA (A sloton belül)
//         let slotStartX, slotStartY, slotCenterX, slotCenterY;
...
//         // 3. MÉRETEZÉS ÉS IGAZÍTÁS
//         // Minden elem belső mérete 1000px (ez a standard)
//         const contentBaseSize = 1000; 
        
//         // Mennyire kell skálázni, hogy a kért CM szélességű legyen?
//         let desiredWidthCm = el.widthCM || 20;
//         let targetSizePx = desiredWidthCm * pxPerCm;
//         let finalScale = targetSizePx / contentBaseSize;
...
//         // Eltolás a slot közepétől
//         // Alapból középre tesszük, aztán eltoljuk a margókkal
//         let transX = slotCenterX - (targetSizePx / 2);
//         let transY = 0;
...
//         // --- 4. LOKÁLIS HÁTTÉR (Fontos: Változóba mentjük a maszkoláshoz!) ---
//         let localBgRect = null;
//         if (el.localBackground) {
//             localBgRect = document.createElementNS("http://www.w3.org/2000/svg", "rect");
//             // Mivel a csoport (g) már skálázva van, itt belső egységeket (1000px) használunk!
//             localBgRect.setAttribute('width', contentBaseSize);
//             localBgRect.setAttribute('height', contentBaseSize);
//             localBgRect.setAttribute('x', 0);
//             localBgRect.setAttribute('y', 0);
            
//             // Gradiens fix (SVG fill vs CSS)
//             if (el.localBackground.includes('gradient')) {
//                 const match = el.localBackground.match(/#[a-fA-F0-9]{6}/);
//                 localBgRect.setAttribute('fill', match ? match[0] : '#000000');
//             } else {
...
//         let needsMasking = (maskType !== 'none');
//         if (el.type === 'photo' && maskType === 'none') needsMasking = true; // Képeknél alapértelmezett kör
...
//             // D) Alkalmazás a Lokális Háttérre (Ha van)
//             if (localBgRect) {
//                 localBgRect.setAttribute('mask', `url(#${maskId})`);
//             }
//         }
...
//             let borderShape;
//             const distOffset = (el.borderDistance || 0); 
//             // A vastagságot vissza kell skálázni, hogy konstans legyen
//             const strokeWidthRaw = (el.borderWidth || 2) / finalScale;
//             const halfStroke = strokeWidthRaw / 2;
//             const borderScale = maskScale + ((distOffset + halfStroke) / 500);
...
//         // --- 8. META ADATOK MENTÉSE (A szövegrendezéshez) ---
//         // Elmentjük a tényleges (transzformált) határokat, hogy a szöveg tudja, hol a térkép
//         el.contentBounds = { 
//             top: transY, 
//             bottom: transY + targetSizePx,
//             // Mentjük a slot határait is, mert a szövegek ahhoz igazodnak alapból
//             slotTop: slotStartY,
//             slotBottom: slotEndY
//         };
...
//     // Okospontok és Szövegek frissítése
//     // (Ha van okospont kód, az ide jöhet)
    
//     window.renderFixedTexts();
// };
// //============================================================
// //--- JAVÍTOTT: refreshMapTransform (Slot határok javítva) ---
// //============================================================
// window.refreshMapTransform = function() {
//     if (!getDOMElements()) return;
//     if (!myCelestialConf.userData) initUserData();
...
//     // --- ELEMEK RENDERELÉSE ---
//     elements.forEach((el, index) => {
//         // 1. POZÍCIÓ SZÁMÍTÁSA (A sloton belül) - ITT VOLT A HIBA
//         let slotStartX, slotStartY, slotCenterX, slotCenterY, slotEndX, slotEndY;
        
//         if (layoutDir === 'column') {
//             slotStartX = 0; 
//             slotEndX = paperW;
//             slotStartY = index * slotH;
//             slotEndY = slotStartY + slotH; // Ezt a változót hiányolta a kód!
...
//             slotStartY = 0;
//             slotEndY = paperH; // Ezt a változót hiányolta a kód!
...
//         // --- 8. META ADATOK MENTÉSE (A szövegrendezéshez) ---
//         // Itt volt a hiba: slotEndY nem volt definiálva, ezért megállt a kód
//         el.contentBounds = { 
//             top: transY, 
//             bottom: transY + targetSizePx,
//             slotTop: slotStartY,
//             slotBottom: slotEndY // Most már létezik
//         };
...
//                          const gapFactor = 0.999;
//                          const maxDim = Math.max(bbox.width, bbox.height);
//                          const viewSize = maxDim / gapFactor; 
//                          const centerX = bbox.x + bbox.width / 2;
//                          const centerY = bbox.y + bbox.height / 2;
...
//                      }
//                  }
//              } catch(e) { console.warn("Auto-fit hiba:", e); }
//         }
//     });

//     // Okospontok (ha vannak)
//     // ... ide jöhet a smartpoints renderelés a régi kódodból ...
//     // De a lényeg, hogy a fenti slotEndY hiba megállította az egész renderelést
//     // mielőtt a szövegek frissülhettek volna.
...
//     elements.forEach((el, index) => {
//         // --- 1. SLOT POZÍCIÓK ---
//         let slotStartX, slotStartY, slotCenterX, slotCenterY, slotEndY;
//         if (layoutDir === 'column') {
//             slotStartX = 0; slotStartY = index * slotH;
//             slotCenterX = paperW / 2; slotCenterY = slotStartY + (slotH / 2);
...
//         // --- 2. LOKÁLIS HÁTTÉR (JAVÍTOTT: SLOT MÉRETRE) ---
//         // Ezt most KÍVÜLRE tesszük a skálázott csoporthoz képest, 
//         // vagy egy külön csoportba, ami nincs skálázva, csak eltolva.
//         if (el.localBackground) {
//             const bgG = document.createElementNS("http://www.w3.org/2000/svg", "g");
//             bgG.setAttribute('transform', `translate(${slotStartX}, ${slotStartY})`); // Slot kezdőpontja
            
//             const bgRect = document.createElementNS("http://www.w3.org/2000/svg", "rect");
//             bgRect.setAttribute('width', slotW);  // TELJES SLOT SZÉLESSÉG
//             bgRect.setAttribute('height', slotH); // TELJES SLOT MAGASSÁG
//             bgRect.setAttribute('x', 0);
//             bgRect.setAttribute('y', 0);
...
//         // --- 3. TARTALOM CSOPORT (Ez a térkép) ---
//         const g = document.createElementNS("http://www.w3.org/2000/svg", "g");
//         g.setAttribute('id', `element-group-${el.id}`);
//         g.setAttribute('class', `designer-element type-${el.type}`);
...
//                 svgNode.setAttribute('width', contentBaseSize);
//                 svgNode.setAttribute('height', contentBaseSize);
//                 svgNode.setAttribute('x', 0); svgNode.setAttribute('y', 0);
//                 svgNode.setAttribute('preserveAspectRatio', 'xMidYMid meet');
//                 svgNode.style.pointerEvents = "none"; 
//                 contentNode = svgNode;
//             }
...
//             contentNode.setAttribute('height', contentBaseSize);
//             contentNode.setAttribute('preserveAspectRatio', 'xMidYMid slice');
//             contentNode.setAttribute('x', 0); svgNode.setAttribute('y', 0);
//         } else {
//             contentNode = document.createElementNS("http://www.w3.org/2000/svg", "rect");
//             contentNode.setAttribute('width', contentBaseSize); contentNode.setAttribute('height', contentBaseSize);
//             contentNode.setAttribute('fill', 'rgba(255,255,255,0.1)');
//         }

//         // --- 5. MASZKOLÁS (A korábbi logika jó) ---
//         const maskType = el.mask || 'none';
//         const maskScale = el.maskScale || 1.0;
//         const edgeBlur = el.edgeBlur || 0;
//         const maskId = `mask-${el.id}`;
...
//         // 2. LOKÁLIS HÁTTÉR (SLOT MÉRETRE)
//         // Ezt külön csoportba tesszük, hogy ne befolyásolja a tartalom transzformációja
//         if (el.localBackground) {
//             const bgG = document.createElementNS("http://www.w3.org/2000/svg", "g");
//             bgG.setAttribute('transform', `translate(${slotStartX}, ${slotStartY})`); // Slot kezdőpontja
...
//         // 3. TARTALOM CSOPORT LÉTREHOZÁSA
//         const g = document.createElementNS("http://www.w3.org/2000/svg", "g");
//         g.setAttribute('id', `element-group-${el.id}`);
//         g.setAttribute('class', `designer-element type-${el.type}`);

//         // 4. MÉRETEZÉS ÉS IGAZÍTÁS
//         const contentBaseSize = 1000; 
//         let desiredWidthCm = el.widthCM || 20;
//         let targetSizePx = desiredWidthCm * pxPerCm;
//         let finalScale = targetSizePx / contentBaseSize;
...
//             // --- ITT VOLT A HIBA: svgNode helyett contentNode kell! ---
//             contentNode.setAttribute('x', 0); 
//             contentNode.setAttribute('y', 0); 
//             // ---------------------------------------------------------
...
// // ============================================================
// // --- JAVÍTOTT: refreshMapTransform (Slot és Node Fix) ---
// // ============================================================
// window.refreshMapTransform = function() {
//     if (!getDOMElements()) return;
//     if (!myCelestialConf.userData) initUserData();
...
//     // --- ELEMEK RENDERELÉSE ---
//     elements.forEach((el, index) => {
//         // 1. POZÍCIÓ (Slot határok definiálása)
//         let slotStartX, slotStartY, slotCenterX, slotCenterY, slotEndX, slotEndY;
...
//         // 2. LOKÁLIS HÁTTÉR (Külön csoportban a sloton)
//         if (el.localBackground) {
//             const bgG = document.createElementNS("http://www.w3.org/2000/svg", "g");
...
//             contentNode.setAttribute('height', contentBaseSize);
//             contentNode.setAttribute('preserveAspectRatio', 'xMidYMid slice');
//             // JAVÍTVA: contentNode-ra hivatkozunk, nem svgNode-ra!
//             contentNode.setAttribute('x', 0); 
//             contentNode.setAttribute('y', 0); 
//         } else {
//             contentNode = document.createElementNS("http://www.w3.org/2000/svg", "rect");
...
//         // 7. BOUNDS MENTÉS (Most már van slotEndY)
//         el.contentBounds = { 
//             top: transY, 
//             bottom: transY + targetSizePx,
//             slotTop: slotStartY,
...
// };
// // ============================================================
// // --- JAVÍTOTT: refreshMapTransform (Gradiens + Slot + NodeFix) ---
// // ============================================================
// window.refreshMapTransform = function() {
//     if (!getDOMElements()) return;
//     if (!myCelestialConf.userData) initUserData();
...
//     const canvasWInput = document.getElementById('canvas-width');
//     const pageWidthCm = parseFloat(canvasWInput && canvasWInput.value ? canvasWInput.value : 21);
//     let effectivePageWidthCm = (layoutDir === 'row' && elements.length > 1) ? pageWidthCm * elements.length : pageWidthCm;
//     const pxPerCm = paperW / effectivePageWidthCm;
...
//         if (layoutDir === 'column') {
//             slotStartX = 0; slotEndX = paperW;
//             slotStartY = index * slotH; slotEndY = slotStartY + slotH;
//             slotCenterX = paperW / 2; slotCenterY = slotStartY + (slotH / 2);
//         } else {
//             slotStartX = index * slotW; slotEndX = slotStartX + slotW;
//             slotStartY = 0; slotEndY = paperH;
//             slotCenterX = slotStartX + (slotW / 2); slotCenterY = paperH / 2;
//         }

//         // 2. LOKÁLIS HÁTTÉR (JAVÍTOTT: GRADIENS TÁMOGATÁS)
//         if (el.localBackground) {
//             const bgG = document.createElementNS("http://www.w3.org/2000/svg", "g");
//             bgG.setAttribute('transform', `translate(${slotStartX}, ${slotStartY})`);
            
//             // Ha gradiens van, foreignObject-et használunk, mert az SVG fill nem szereti a CSS stringeket
//             if (el.localBackground.includes('gradient') || el.localBackground.includes('url')) {
//                 const fo = document.createElementNS("http://www.w3.org/2000/svg", "foreignObject");
//                 fo.setAttribute('width', slotW);
//                 fo.setAttribute('height', slotH);
//                 fo.setAttribute('x', 0);
//                 fo.setAttribute('y', 0);
                
//                 // Beágyazott HTML DIV a háttérrel
//                 const div = document.createElement('div');
//                 div.style.width = '100%';
//                 div.style.height = '100%';
//                 div.style.background = el.localBackground; // Itt működik a linear-gradient!
                
//                 fo.appendChild(div);
//                 bgG.appendChild(fo);
//             } else {
//                 // Sima színnél marad a rect (jobb teljesítmény)
//                 const bgRect = document.createElementNS("http://www.w3.org/2000/svg", "rect");
//                 bgRect.setAttribute('width', slotW);  
//                 bgRect.setAttribute('height', slotH);
//                 bgRect.setAttribute('fill', el.localBackground);
...
//             contentNode.setAttribute('height', contentBaseSize);
//             contentNode.setAttribute('preserveAspectRatio', 'xMidYMid slice');
//             contentNode.setAttribute('x', 0); contentNode.setAttribute('y', 0); 
//         } else {
//             contentNode = document.createElementNS("http://www.w3.org/2000/svg", "rect");
//             contentNode.setAttribute('width', contentBaseSize); 
//             contentNode.setAttribute('height', contentBaseSize);
...
// };
// // ============================================================
// // --- JAVÍTOTT: refreshMapTransform (Auto-fit visszaépítve) ---
// // ============================================================
// window.refreshMapTransform = function() {
//     if (!getDOMElements()) return;
//     if (!myCelestialConf.userData) initUserData();
...
//     const canvasWInput = document.getElementById('map-width-cm-input');
//     const pageWidthCm = parseFloat(canvasWInput && canvasWInput.value ? canvasWInput.value : 21);
//     let effectivePageWidthCm = (layoutDir === 'row' && elements.length > 1) ? pageWidthCm * elements.length : pageWidthCm;
//     const pxPerCm = paperW / effectivePageWidthCm;
...
//                 div.style.width = '100%';
//                 div.style.height = '100%';
//                 div.style.background = el.localBackground;
...
//                 svgNode.setAttribute('preserveAspectRatio', 'xMidYMid meet');
//                 svgNode.style.pointerEvents = "none"; 
//                 // ID beállítása, hogy később megtaláljuk az Auto-fithez
//                 svgNode.setAttribute('id', `embedded-map-${el.id}`);
//                 contentNode = svgNode;
//             }
//         } else if (el.dataURL) {
...
//         // --- 8. VISSZAÉPÍTETT AUTO-FIT LOGIKA ---
//         if (el.type === 'map') {
//              try {
//                  // Meg kell keresni az imént hozzáadott SVG-t a DOM-ban
//                  const realSvg = document.getElementById(`embedded-map-${el.id}`);
//                  const bg = g.querySelector('.background'); 
...
// // ============================================================
// // --- JAVÍTOTT: refreshMapTransform (Helyes ID + Auto-fit) ---
// // ============================================================
// window.refreshMapTransform = function() {
//     if (!getDOMElements()) return;
//     if (!myCelestialConf.userData) initUserData();
...
//     // --- JAVÍTÁS: HELYES ID HASZNÁLATA ---
//     let canvasWInput = document.getElementById('map-width-cm-input');
//     if (!canvasWInput) canvasWInput = document.getElementById('canvas-width');
...
//             } else {
//                 const bgRect = document.createElementNS("http://www.w3.org/2000/svg", "rect");
//                 bgRect.setAttribute('width', slotW); bgRect.setAttribute('height', slotH);
//                 bgRect.setAttribute('fill', el.localBackground);
//                 bgG.appendChild(bgRect);
//             }
//             renderLayer.appendChild(bgG);
...
//         el.contentBounds = { 
//             top: transY, bottom: transY + targetSizePx,
//             slotTop: slotStartY, slotBottom: slotEndY 
//         };

//         g.style.cursor = "pointer";
//         g.onclick = function(e) {
//             const uiState = myCelestialConf.userData.uiState;
//             if (uiState.placingSmartpoint || uiState.movingSmartpointId) { window.handleCanvasClick(e); e.stopPropagation(); return; }
//             e.stopPropagation();
//             if (el.type === 'map') { window.loadMapToEditor(el.id); if(window.handleElementSelection) window.handleElementSelection(el.id); }
//             else { window.highlightPhoto(el.id, this); if(window.handleElementSelection) window.handleElementSelection(el.id); }
//         };
...
// };
// // ============================================================
// // --- JAVÍTOTT: refreshMapTransform (Skálázás + Auto-fit) ---
// // ============================================================
// window.refreshMapTransform = function() {
//     if (!getDOMElements()) return;
//     if (!myCelestialConf.userData) initUserData();
...
//     // --- 1. SKÁLÁZÁS JAVÍTÁSA ---
//     // A vászon "pixel per cm" arányát a VÁSZON méretéből számoljuk (pl. 21cm),
//     // NEM a térkép méretéből. Így nem lesz fordított méretezés.
//     let canvasWInput = document.getElementById('canvas-width'); 
//     // Ha nincs canvas input, alapértelmezett A4 (21cm)
//     const pageWidthCm = parseFloat(canvasWInput && canvasWInput.value ? canvasWInput.value : 21);
...
//             if (el.localBackground.includes('gradient') || el.localBackground.includes('url')) {
//                 const fo = document.createElementNS("http://www.w3.org/2000/svg", "foreignObject");
//                 fo.setAttribute('width', slotW); fo.setAttribute('height', slotH);
//                 fo.setAttribute('x', 0); fo.setAttribute('y', 0);
//                 const div = document.createElement('div');
//                 div.style.width = '100%'; div.style.height = '100%';
//                 div.style.background = el.localBackground;
//                 fo.appendChild(div); bgG.appendChild(fo);
//             } else {
//                 const bgRect = document.createElementNS("http://www.w3.org/2000/svg", "rect");
//                 bgRect.setAttribute('width', slotW); bgRect.setAttribute('height', slotH);
//                 bgRect.setAttribute('fill', el.localBackground);
...
//         // --- MÉRETEZÉS (EGYEDI ELEM SZINTEN) ---
//         const contentBaseSize = 1000; 
//         // Itt olvassuk ki az EGYEDI méretet. Ha nincs megadva, default 20.
//         let desiredWidthCm = el.widthCM || 20; 
...
// };
// ============================================================
// --- JAVÍTOTT: refreshMapTransform (Rect háttér + Width fix) ---
// ============================================================
window.refreshMapTransform = function() {
    if (!getDOMElements()) return;
    if (!myCelestialConf.userData) initUserData();
    ...
    elements.forEach((el, index) => {
        ...
        // 2. LOKÁLIS HÁTTÉR (VISSZAÁLLÍTVA <rect>-RE!)
        if (el.localBackground) {
            const bgG = document.createElementNS("http://www.w3.org/2000/svg", "g");
            bgG.setAttribute('transform', `translate(${slotStartX}, ${slotStartY})`);
            ...
            // Itt visszaálltunk a sima fill-re, ahogy kérted (nincs div)
            // Ha gradiens kell SVG-ben, azt <defs>-ben kellene definiálni, 
            // de a kérés szerint legyen "mint az eredeti".
            bgRect.setAttribute('fill', el.localBackground);
        ...
        // --- MÉRETEZÉS FIX (AUTOMATIKUS 21CM HA HIÁNYZIK) ---
        const contentBaseSize = 1000; 
        
        // Ha nincs beállítva szélesség, beállítjuk az alapértelmezettet és MENTJÜK!
        if (!el.widthCM) {
            el.widthCM = 21; // Alapértelmezett méret
        }
        
        let targetSizePx = el.widthCM * pxPerCm;
        let finalScale = targetSizePx / contentBaseSize;
...
// ============================================================
// --- HIÁNYZÓ SNAPSHOTOK PÓTLÁSA (Új térképekhez) ---
// ============================================================
window.generateMissingSnapshots = function() {
    if (!myCelestialConf.userData) return;
    
    // Keressük azokat a térképeket, amiknek nincs vectorData-ja
    const missingMaps = myCelestialConf.userData.elements.filter(el => el.type === 'map' && !el.vectorData);
    
    if (missingMaps.length > 0) {
        console.log(`⚠️ ${missingMaps.length} térképnek hiányzik a vektoros adata. Generálás indítása...`);
        // Elindítjuk rájuk a renderelő sort (csak generál, nem színez át semmit)
        
        // Előtte biztosítjuk a configot
        missingMaps.forEach(el => {
            if (!el.celestialConfig) {
                // Ha nincs config, csinálunk egy alap másolatot a jelenlegiből
                let baseConfig = JSON.parse(JSON.stringify(myCelestialConf));
                if (baseConfig.userData) delete baseConfig.userData;
                baseConfig.width = 1000;
                el.celestialConfig = baseConfig;
            }
        });

        // Queue indítása
        processRenderQueue(missingMaps, 0);
    } else {
        // Ha nincs hiányzó, csak frissítjük a nézetet
...
window.setMapWidth = function(val) {
    ...
    // Megkeressük a kijelölt elemet
    const el = myCelestialConf.userData.elements.find(e => e.id == uiState.selectedElementId);
    if (el) {
        // Frissítjük a szélességet
        el.widthCM = parseFloat(val);
        // Újrarajzoljuk a tervezőt (Most már helyes skálázással!)
        window.refreshMapTransform();
    }
...
window.updateElementSelectorUI = function() {
    ...
    const uiState = myCelestialConf.userData.uiState;
    // Ha nincs kijelölt, üres
    if (!uiState.selectedElementId) {
        $('#map-width-cm-input').val('');
        return;
    }
    const el = myCelestialConf.userData.elements.find(e => e.id == uiState.selectedElementId);
    if (el) {
        // Beállítjuk az input értékét az elem szélességére (vagy default 20)
        $('#map-width-cm-input').val(el.widthCM || 20);
    }
...
// // ============================================================
// // --- 1. VEKTOROS ADAT FOGADÁSA (Szinkronizált Mentés) ---
// // ============================================================
// window.handleVectorExport = function(svgContent) {
//     console.log("✅ Vektoros adat (SVG) érkezett feldolgozásra...");
...
//     // 1. Cél elem azonosítása (Most már a mentett ID-ből, ami biztosabb)
//     let targetId = myCelestialConf.userData.uiState.selectedElementId;
//     let targetEl = myCelestialConf.userData.elements.find(e => e.id == targetId && e.type === 'map');

//     if (targetEl) {
//         // --- CSS OSZTÁLYOK EGYEDIVÉ TÉTELE (SCOPING) ---
//         let scopedSvg = svgContent;
//         const uniquePrefix = `map_${targetEl.id}_`; 

//         // Style blokkban lévő osztályok cseréje
//         const styleMatch = scopedSvg.match(/<style[^>]*>([\s\S]*?)<\/style>/i);
//         if (styleMatch) {
//             let originalCss = styleMatch[1];
//             // Az opacity védelmével (nem cseréljük a számokat)
//             let newCss = originalCss.replace(/(\.)(?!\d)([a-zA-Z0-9_-]+)/g, (match, dot, className) => {
//                 return `.${uniquePrefix}${className}`;
//             });
//             scopedSvg = scopedSvg.replace(originalCss, newCss);
...
//         // HTML elemek class attribútumainak cseréje
//         scopedSvg = scopedSvg.replace(/class="([^"]+)"/g, (match, classNames) => {
//             const newClassNames = classNames.split(' ').map(c => uniquePrefix + c).join(' ');
//             return `class="${newClassNames}"`;
//         });

//         // Background osztály helyreállítása (hogy a méretező megtalálja)
//         scopedSvg = scopedSvg.replace(new RegExp(`class="[^"]*${uniquePrefix}background[^"]*"`, 'g'), (match) => {
//             return match.slice(0, -1) + ' background"';
//         });

//         // ID-k és URL hivatkozások cseréje
//         scopedSvg = scopedSvg.replace(/id="([^"]+)"/g, (match, idVal) => {
//             return `id="${uniquePrefix}${idVal}"`;
//         });
//         scopedSvg = scopedSvg.replace(/url\(#([^)]+)\)/g, (match, idRef) => {
...
//         console.log(`✅ Térkép (ID: ${targetEl.id}) sikeresen mentve.`);
        
//         // UI Frissítés (Csak a nézetet)
//         window.refreshMapTransform();
        
//     } else {
//         console.warn("❌ Hiba: Nem találtam a cél térképet az SVG mentéséhez.");
//     }
...
//         window.onVectorExportFinished = null; // Töröljük, hogy ne fusson le kétszer
//         callback(); // Hívjuk a következő lépést
//     }
// };

window.handleVectorExport = function(svgContent) {
    // ... (A CSS izoláló kódok változatlanok, maradhatnak a régiek) ...
    // A lényeg a mentés és a callback hívása:

    if (!myCelestialConf.userData) initUserData(); // Biztonsági init

    // Mivel a processRenderQueue "becsapta" a userDatát, az egyetlen elem, 
    // ami benne van az 'elements' tömbben, az a cél elemünk!
    ...
    if (targetEl) {
        ...
        // --- MENTÉS AZ EREDETI (elmentett) OBJEKTUMBA IS ---
        // Mivel a currentEl egy referencia, ha itt módosítjuk, az az eredeti tömbben is módosul!
        targetEl.vectorData = scopedSvg;
...
//     console.log("window.applyDesignerTheme = function(key, variant = 'normal') {");
...
window.applyDesignerTheme = function(key, variant = 'normal') {
    // Csak meghívjuk a fő függvényt, az intéz mindent
...
// window.applyDesignerTheme = function(key, variant = 'normal') {
//     console.log("window.applyDesignerTheme = function(key, variant = 'normal') {");
//     // 1. Téma betöltése (Ez intézi a hátteret és a térkép konfigurációt is)
//     if (typeof window.loadTheme === 'function') {
//         window.loadTheme(key, variant); 
//     }

//     // 2. Frissítések (Redraw)
//     if (typeof Celestial !== 'undefined') {
//         setTimeout(() => {
//             try { Celestial.redraw(); } catch (e) { console.warn("Redraw error", e); }
            
//             // Snapshot és transzformációk frissítése
//             setTimeout(() => {
//                 // Csak akkor csinálunk snapshotot, ha a loadTheme nem csinálta volna meg
//                 // (De a loadTheme most már megcsinálja, így ez inkább csak biztonsági háló)
//                 // window.updateActiveMapSnapshot(); 
...
// // ============================================================
// // --- 3. A FŐ TÉMA BETÖLTŐ (Minden hibajavítással) ---
// // ============================================================
// window.loadTheme = function(key, variant = 'normal', sourceSide = 'right') {
//     console.log(`🎨 Téma betöltése: ${key} (${variant}) innen: ${sourceSide}`);
    
//     // 1. AKTÍV ELEM KERESÉSE
//     if (!myCelestialConf.userData) initUserData();
//     const uiState = myCelestialConf.userData.uiState;
//     let targetId = uiState.selectedElementId;
//     let targetEl = myCelestialConf.userData.elements.find(e => e.id == targetId);

//     // Ha nincs kijelölt vagy nem térkép, az első térképet vesszük
//     if (!targetEl || targetEl.type !== 'map') {
//         targetEl = myCelestialConf.userData.elements.find(e => e.type === 'map');
//         if (targetEl) targetId = targetEl.id;
//     }

//     if (!targetEl) {
//         alert("Nincs kiválasztva csillagtérkép!");
//         return;
//     }
...
//     let newConfig = JSON.parse(JSON.stringify(theme.config));
    
//     // *** KULCSFONTOSSÁGÚ: Kényszerített méret (1000px) ***
//     // Ez akadályozza meg, hogy a több térkép esetén szétnyúljon
//     newConfig.width = 1000; 

//     // Projekció
//     if (variant === 'heart') {
//         newConfig.projection = "customHeart"; 
//         targetEl.mask = 'classic'; 
//     } else {
//         newConfig.projection = "stereographic";
//     }

//     // 3. HÁTTÉR KEZELÉS (FORRÁS ALAPJÁN)
//     // Most már pontosan tudjuk, melyik gombcsoportot kell nézni!
//     let bgMode = 'global'; // Alapértelmezett fallback
    
//     let radioName = 'bg-mode-right'; // Default designer
//     if (sourceSide === 'left') radioName = 'bg-mode-left';
    
//     // Megkeressük a kiválasztott rádiógombot a megfelelő névvel
//     const selectedRadio = document.querySelector(`input[name="${radioName}"]:checked`);
//     if (selectedRadio) {
//         bgMode = selectedRadio.value;
//     }

//     console.log(`Háttér mód (${radioName}):`, bgMode);
    
//     const themeBg = theme.background;

//     if (bgMode === 'global') {
//         // 🌍 GLOBÁLIS: Vászon + Töröljük a lokálist
//         window.updateCanvasBackground(themeBg);
//         targetEl.localBackground = null; 
//     } 
//     else if (bgMode === 'local') {
//         // 🖼️ LOKÁLIS: Csak az elembe mentjük
//         targetEl.localBackground = themeBg;
//     }
//     else {
//         // 🚫 NONE: Nem nyúlunk semmihez
//         console.log("Háttér változtatás kihagyva.");
//     }

//     // 4. MENTÉS AZ ELEMBE
//     if (targetEl.celestialConfig) {
//         newConfig.Ido = targetEl.celestialConfig.Ido || newConfig.Ido;
//         newConfig.Varos = targetEl.celestialConfig.Varos || newConfig.Varos;
//         newConfig.Lokacio = targetEl.celestialConfig.Lokacio || newConfig.Lokacio;
//         newConfig.geopos = targetEl.celestialConfig.geopos || newConfig.geopos;
//     }
//     targetEl.celestialConfig = newConfig;

//     // Szövegszínek
//     const zoneKey = (targetId === 'main-map') ? 'map' : `map_${targetId}`;
//     const zones = myCelestialConf.userData.zones;
//     if (theme.textColor && zones[zoneKey]) {
...
//     // 5. MOTOR FRISSÍTÉSE ÉS KIRAJZOLÁS (Kényszerített 1000px-en)
//     myCelestialConf = JSON.parse(JSON.stringify(newConfig));
//     myCelestialConf.userData = uiState.parentUserData || window.myCelestialConf.userData; 

//     if (typeof window.updateGUIFromConfig === 'function') {
//         window.updateGUIFromConfig(newConfig);
//     }

//     if (typeof Celestial !== 'undefined') {
//         // 1. MÉRET RESET (Biztonsági) - Ez akadályozza meg az összeolvadást!
//         if (typeof Celestial.resize === 'function') Celestial.resize({width: 1000}); 

//         // 2. APPLY & REDRAW
//         Celestial.apply(newConfig);
        
//         if (newConfig.Ido) Celestial.date(new Date(newConfig.Ido));
//         if (newConfig.geopos) Celestial.location(newConfig.geopos);
        
//         Celestial.redraw();
//     }

//     // 6. SNAPSHOT & UI FRISSÍTÉS
//     setTimeout(() => {
//         // Ez menti el a képet az elembe
//         window.updateActiveMapSnapshot(); 
        
//         // Helyretesszük a tervezőben
//         window.refreshMapTransform();
//         window.renderFixedTexts();
//         if(window.renderZoneUI) {
//             window.renderZoneUI('top');
...
//         // Kijelölés frissítése (mindkét panelen, hogy látszódjon)
//         $('.theme-item').removeClass('active-theme');
//         const activeBtns = document.querySelectorAll(`.theme-btn[data-key="${key}"][data-variant="${variant}"]`);
//         activeBtns.forEach(btn => btn.closest('.theme-item').classList.add('active-theme'));
        
//     }, 150); 
// };
// window.loadTheme = function(key, variant = 'normal', sourceSide = 'right') {
//     console.log(`🎨 Téma betöltése: ${key} (${variant}) - Forrás: ${sourceSide}`);
...
//     if (!targetEl) { alert("Nincs kiválasztva csillagtérkép!"); return; }
...
//     // *** MÉRET KÉNYSZERÍTÉS ***
//     // Ez akadályozza meg, hogy a generátor "szétfolyjon" a teljes vászonra
//     newConfig.width = 1000; 
...
//         ['top', 'bottom'].forEach(z => {
//             if (zones[zoneKey][z]) {
//                 zones[zoneKey][z].blocks.forEach(block => { block.color = theme.textColor; });
//             }
//         });
//     }
...
//     if (typeof window.updateGUIFromConfig === 'function') window.updateGUIFromConfig(newConfig);

//     if (typeof Celestial !== 'undefined') {
//         // 1. MÉRET RESET (Kényszerítve)
//         if (typeof Celestial.resize === 'function') Celestial.resize({width: 1000}); 
...
//     // 6. SNAPSHOT & UI FRISSÍTÉS (Kicsit hosszabb timeout a biztonságért)
//     setTimeout(() => {
//         // Ez menti el a képet (ami most már 1000px széles) az elembe
//         window.updateActiveMapSnapshot(); 
        
//         // Helyretesszük (A refreshMapTransform már jól kezeli a méretet és a hátteret is)
//         window.refreshMapTransform();
//         window.renderFixedTexts();
//         if(window.renderZoneUI) {
//             window.renderZoneUI('top');
...
// // ============================================================
// // --- TÖKÉLETESÍTETT TÉMA BETÖLTÉS (IZOLÁLT RENDERELÉS) ---
// // ============================================================
// window.loadTheme = function(key, variant = 'normal', sourceSide = 'right') {
//     console.log(`🎨 Téma betöltése: ${key} (${variant}) - Forrás: ${sourceSide}`);
...
//     // Ha nincs kijelölt vagy nem térkép, keressük az első térképet
//     if (!targetEl || targetEl.type !== 'map') {
//         targetEl = myCelestialConf.userData.elements.find(e => e.type === 'map');
//         if (targetEl) targetId = targetEl.id;
//     }
...
//     // *** KÉNYSZERÍTETT BELSŐ MÉRET ***
//     // Ez a titok: Mindig 1000px-ben gondolkodunk generáláskor.
//     // Nem érdekel minket a vászon mérete, azt majd a refreshMapTransform intézi.
//     newConfig.width = 1000; 
...
//     } else {
//         newConfig.projection = "stereographic";
//         // Ha vissza akarod állítani körre normál nézetben, vedd ki a kommentet:
//         // targetEl.mask = 'none'; 
//     }

//     // 3. HÁTTÉR KEZELÉS (FORRÁS ALAPJÁN)
//     let bgMode = 'global'; 
//     // Megkeressük a megfelelő rádiógombot a forrás oldalon
//     let radioName = (sourceSide === 'left') ? 'bg-mode-left' : 'bg-mode-right';
...
//     if (bgMode === 'global') {
//         // 🌍 GLOBÁLIS: A vászon hátterét állítjuk, a lokálist töröljük
//         window.updateCanvasBackground(themeBg);
//         targetEl.localBackground = null; 
//     } 
//     else if (bgMode === 'local') {
//         // 🖼️ LOKÁLIS: Csak az elem kapja meg
//         targetEl.localBackground = themeBg;
//     }
//     // 'none' esetén nem csinálunk semmit

//     // 4. ADATOK MENTÉSE (Hely, Idő megőrzése)
//     if (targetEl.celestialConfig) {
//         newConfig.Ido = targetEl.celestialConfig.Ido || newConfig.Ido;
//         newConfig.Varos = targetEl.celestialConfig.Varos || newConfig.Varos;
//         newConfig.Lokacio = targetEl.celestialConfig.Lokacio || newConfig.Lokacio;
...
//     // 5. MOTOR FRISSÍTÉSE ÉS KIRAJZOLÁS (IZOLÁLTAN)
//     // Ideiglenesen frissítjük a globális configot a generáláshoz
//     const savedGlobalUserData = myCelestialConf.userData; // Megmentjük a userDatát
//     myCelestialConf = JSON.parse(JSON.stringify(newConfig));
//     myCelestialConf.userData = savedGlobalUserData; // Visszatesszük
...
//     if (typeof Celestial !== 'undefined') {
//         // 1. MÉRET RESET (Kényszerítve 1000px-re)
//         if (typeof Celestial.resize === 'function') Celestial.resize({width: 1000}); 
...
//         // Helyretesszük (Méretezés és Pozíció)
//         window.refreshMapTransform();
//         window.renderFixedTexts();
//         if(window.renderZoneUI) {
//             window.renderZoneUI('top');
...
//         // Kijelölés frissítése a listában
//         $('.theme-item').removeClass('active-theme');
//         const activeBtns = document.querySelectorAll(`.theme-btn[data-key="${key}"][data-variant="${variant}"]`);
//         activeBtns.forEach(btn => btn.closest('.theme-item').classList.add('active-theme'));
...
//     // Ha nincs kijelölt, keressük az első térképet
...
//     // Alap config klónozása
//     let themeConfig = JSON.parse(JSON.stringify(theme.config));
//     themeConfig.width = 1000; // Kényszerített generálási méret

//     // PROJEKCIÓ KEZELÉSE (VISSZAÁLLÍTVA A CUSTOMHEART-RA)
//     if (variant === 'heart') {
//         themeConfig.projection = "customHeart"; // Ezt használjuk, nem a stereographic-ot!
//         // A maszk marad, hogy szép legyen a széle
//         selectedEl.mask = 'classic'; 
//     } else {
//         themeConfig.projection = "stereographic";
//         // selectedEl.mask = 'none'; // Ha akarod, itt leveheted a maszkot
//     }
...
//     // --- LOGIKA SZÉTVÁLASZTÁSA ---

//     if (bgMode === 'global') {
//         // 🌍 GLOBÁLIS: Minden térkép, Globális háttér, Minden szöveg
        
//         // Háttér: Vászon beállítása, lokálisok törlése
//         window.updateCanvasBackground(themeBg);
//         myCelestialConf.userData.elements.forEach(el => el.localBackground = null);

//         // Elemek: Mindegyik térképet frissítjük
//         myCelestialConf.userData.elements.filter(el => el.type === 'map').forEach(el => {
//             elementsToUpdate.push(el);
//         });

//     } else if (bgMode === 'local') {
//         // 🖼️ LOKÁLIS: Csak kijelölt térkép, Lokális slot háttér, Kijelölt szöveg
        
//         // Háttér: Csak az elembe mentjük
//         selectedEl.localBackground = themeBg;

//         // Elemek: Csak a kiválasztott
//         elementsToUpdate.push(selectedEl);

//     } else if (bgMode === 'none') {
//         // 🚫 NO BG: Csak kijelölt térkép (stílus), Nincs háttér váltás, Nincs szöveg váltás
        
//         // Elemek: Csak a kiválasztott
//         elementsToUpdate.push(selectedEl);
//     }

//     // --- ALKALMAZÁS A MEGHATÁROZOTT ELEMEKRE ---
    
//     // Ideiglenes mentés a loophoz
//     const savedGlobalUserData = myCelestialConf.userData;

//     // Loop az érintett elemeken (Global = mind, Local/None = 1 db)
//     // Aszinkron loopot szimulálunk, hogy a Celestialnak legyen ideje rajzolni
//     let idx = 0;
    
//     function processNextElement() {
//         if (idx >= elementsToUpdate.length) {
//             // Ha végeztünk mindennel
//             myCelestialConf.userData = savedGlobalUserData; // Visszaállítjuk a userDatát
...
//         const el = elementsToUpdate[idx];
        
//         // 1. Config egyesítése a meglévő adatokkal (Hely, Idő megőrzése)
//         let elConfig = JSON.parse(JSON.stringify(themeConfig));
//         if (el.celestialConfig) {
//             elConfig.Ido = el.celestialConfig.Ido || elConfig.Ido;
//             elConfig.Varos = el.celestialConfig.Varos || elConfig.Varos;
//             elConfig.Lokacio = el.celestialConfig.Lokacio || elConfig.Lokacio;
//             elConfig.geopos = el.celestialConfig.geopos || elConfig.geopos;
//         }
//         el.celestialConfig = elConfig;

//         // 2. Szövegszínek (CSAK HA NEM 'NONE' MÓD!)
//         if (bgMode !== 'none') {
//             const zoneKey = (el.id === 'main-map' || !el.id) ? 'map' : `map_${el.id}`;
//             const zones = savedGlobalUserData.zones;
//             if (theme.textColor && zones[zoneKey]) {
//                 ['top', 'bottom'].forEach(z => {
//                     if (zones[zoneKey][z]) {
//                         zones[zoneKey][z].blocks.forEach(block => { block.color = theme.textColor; });
...
//         // 3. Generálás
//         // Beállítjuk a globális változót, hogy a Celestial ezt lássa
//         myCelestialConf = elConfig; 
        
//         if (typeof window.updateGUIFromConfig === 'function' && el.id === targetId) {
//             window.updateGUIFromConfig(elConfig);
//         }

//         if (typeof Celestial !== 'undefined') {
//             Celestial.resize({width: 1000}); // Kényszerített méret
//             Celestial.apply(elConfig);
//             if (elConfig.Ido) Celestial.date(new Date(elConfig.Ido));
//             if (elConfig.geopos) Celestial.location(elConfig.geopos);
//             Celestial.redraw();
//         }

//         // 4. Snapshot mentése
//         setTimeout(() => {
//             // A Celestial.exportSVG-t hívjuk, de a handleVectorExport-ot "átverjük", 
//             // hogy tudja, melyik elemnek szánjuk a snapshotot
            
//             // Ideiglenesen felülírjuk a kijelölt elemet a userDatában, 
//             // hogy a handleVectorExport a jót frissítse (ha globális módban vagyunk)
//             savedGlobalUserData.uiState.selectedElementId = el.id;
//             myCelestialConf.userData = savedGlobalUserData;

//             window.updateActiveMapSnapshot(); // Ez menti el az el.vectorData-ba

//             idx++;
//             processNextElement(); // Következő elem
//         }, 150);
//     }
...
// // ============================================================
// // --- TÖKÉLETESÍTETT TÉMA BETÖLTÉS (SZINKRONIZÁLT LOOP) ---
// // ============================================================
// window.loadTheme = function(key, variant = 'normal', sourceSide = 'right') {
//     console.log(`🎨 Téma betöltése: ${key} (${variant}) - Forrás: ${sourceSide}`);
...
//     // 1. KIJELÖLT ELEM KERESÉSE
//     const uiState = myCelestialConf.userData.uiState;
//     let originalTargetId = uiState.selectedElementId;
//     let selectedEl = myCelestialConf.userData.elements.find(e => e.id == originalTargetId);

//     // Ha nincs (vagy nem térkép), keressük az első térképet
//     if (!selectedEl || selectedEl.type !== 'map') {
//         selectedEl = myCelestialConf.userData.elements.find(e => e.type === 'map');
//         if (selectedEl) originalTargetId = selectedEl.id;
//     }
...
//     // 3. MÓD KIVÁLASZTÁSA (Rádiógombok alapján)
//     let bgMode = 'global'; 
//     let radioName = (sourceSide === 'left') ? 'bg-mode-left' : 'bg-mode-right';
//     const selectedRadio = document.querySelector(`input[name="${radioName}"]:checked`);
//     if (selectedRadio) bgMode = selectedRadio.value;
...
//         // 🌍 GLOBÁLIS
//         window.updateCanvasBackground(themeBg);
//         // Mindenkinek töröljük a lokálisát
//         myCelestialConf.userData.elements.forEach(el => el.localBackground = null);
        
//         // Minden térképet újra kell generálni
//         myCelestialConf.userData.elements.filter(el => el.type === 'map').forEach(el => {
...
//         // 🖼️ LOKÁLIS
//         selectedEl.localBackground = themeBg;
//         elementsToProcess.push(selectedEl); // Csak ezt generáljuk újra

//     } else if (bgMode === 'none') {
//         // 🚫 CSAK STÍLUS
//         // Nem bántjuk a hátteret sehol
//         elementsToProcess.push(selectedEl); // Csak ezt generáljuk újra
//     }

//     // Mentjük a globális állapotot, mert a generálás alatt átírjuk
//     const savedGlobalUserData = JSON.parse(JSON.stringify(myCelestialConf.userData));

//     // --- SZINKRON LOOP INDÍTÁSA ---
//     let processIndex = 0;

//     function processNext() {
//         // Ha végeztünk a listával
//         if (processIndex >= elementsToProcess.length) {
//             console.log("🏁 Minden térkép generálása kész.");
            
//             // Visszaállítjuk az eredeti kijelölést és adatokat
//             myCelestialConf.userData = savedGlobalUserData;
...
//         const currentEl = elementsToProcess[processIndex];
//         console.log(`⏳ Generálás indítása: ${currentEl.id} (${processIndex + 1}/${elementsToProcess.length})`);

//         // 1. Config összeállítása az adott elemre
//         let elConfig = JSON.parse(JSON.stringify(themeConfig));
        
//         // Megőrizzük az elem saját adatait (Hely, Idő, Koord)
//         if (currentEl.celestialConfig) {
//             elConfig.Ido = currentEl.celestialConfig.Ido || elConfig.Ido;
//             elConfig.Varos = currentEl.celestialConfig.Varos || elConfig.Varos;
//             elConfig.Lokacio = currentEl.celestialConfig.Lokacio || elConfig.Lokacio;
//             elConfig.geopos = currentEl.celestialConfig.geopos || elConfig.geopos;
//         }
//         currentEl.celestialConfig = elConfig;

//         // 2. Szövegszínek frissítése (ha nem 'none' mód)
//         if (bgMode !== 'none') {
//             const zoneKey = (currentEl.id === 'main-map' || !currentEl.id) ? 'map' : `map_${currentEl.id}`;
//             const zones = savedGlobalUserData.zones; // A mentett zónákat módosítjuk
//             if (theme.textColor && zones[zoneKey]) {
//                 ['top', 'bottom'].forEach(z => {
//                     if (zones[zoneKey][z]) {
//                         zones[zoneKey][z].blocks.forEach(block => { block.color = theme.textColor; });
...
//         // 3. Globális változók beállítása a Celestialnak
//         // Fontos: A userDatát úgy állítjuk be, hogy a selectedElementId az aktuális elem legyen!
//         // Így a handleVectorExport tudni fogja, hova kell menteni.
//         myCelestialConf = elConfig;
//         // myCelestialConf.userData = { 
//         //     uiState: { selectedElementId: currentEl.id },
//         //     elements: [currentEl] // Csak technikai okokból, hogy létezzen
//         // };
//         // myCelestialConf.userData = { 
//         //     uiState: { selectedElementId: currentEl.id },
//         //     elements: [currentEl], // Csak technikai okokból
//         //     zones: savedGlobalUserData.zones // <--- EZ HIÁNYZOTT! Enélkül omlik össze.
//         // };
//         myCelestialConf.userData = { 
//             uiState: { 
//                 selectedElementId: currentEl.id,
//                 zoneFlags: savedGlobalUserData.uiState.zoneFlags || {} // <--- PÓTOLVA: zoneFlags
//             },
//             elements: [currentEl],
//             zones: savedGlobalUserData.zones, // <--- PÓTOLVA: zones
//             canvas: savedGlobalUserData.canvas // <--- PÓTOLVA: canvas (EZ OKOZTA A HIBÁT)
//         };

//         // GUI frissítés (csak ha ez az eredetileg kijelölt elem)
//         if (currentEl.id === originalTargetId && typeof window.updateGUIFromConfig === 'function') {
//             window.updateGUIFromConfig(elConfig);
//         }

//         // 4. Celestial Generálás
//         if (typeof Celestial !== 'undefined') {
//             Celestial.resize({width: 1000}); // Kényszerített méret
//             Celestial.apply(elConfig);
//             if (elConfig.Ido) Celestial.date(new Date(elConfig.Ido));
...
//             // Kiemelések visszaállítása (ha van mentve)
//             if (currentEl.highlights) Celestial.highlightList = currentEl.highlights;
//             else Celestial.highlightList = {};

//             Celestial.redraw();
//         }

//         // 5. Snapshot indítása (Callback beállítása)
//         // Beállítjuk, hogy mi fusson le, ha kész a mentés
//         window.onVectorExportFinished = function() {
//             processIndex++;
//             processNext(); // Jöhet a következő
//         };

//         // Pici késleltetés, hogy a redraw biztosan végezzen a DOM-ban
//         setTimeout(() => {
//             window.updateActiveMapSnapshot();
//         }, 100);
//     }
...
// // ============================================================
// // --- RENDER QUEUE (A MEMÓRIA SZIVÁRGÁS ELLEN) ---
// // ============================================================
// function processRenderQueue(elementsList, index) {
//     // 1. KILÉPÉSI FELTÉTEL: Ha végigértünk a listán
//     if (index >= elementsList.length) {
//         console.log("✅ Minden térkép generálása kész.");
        
//         // Takarítás: Visszaállítjuk a UserDatát az eredeti állapotra
//         // (A renderelés alatt csonkolva volt a biztonság kedvéért)
...
//     // 2. ELŐKÉSZÜLETEK A KÖVETKEZŐ ELEMHEZ
//     const el = elementsList[index];
//     console.log(`⏳ Generálás: ${el.id} (${index + 1}/${elementsList.length})`);

//     // Elmentjük a teljes userDatát, ha még nem tettük (a ciklus elején)
//     if (index === 0 && !window.savedFullUserData) {
//         window.savedFullUserData = myCelestialConf.userData;
//     }

//     // 3. CELESTIAL MOTOR KONFIGURÁLÁSA (Izolált környezet)
//     // Ideiglenesen "becsapjuk" a rendszert, hogy csak ez az egy elem létezik.
//     // Így a generátor nem akad össze a többi elemmel.
//     myCelestialConf = JSON.parse(JSON.stringify(el.celestialConfig));
//     myCelestialConf.userData = {
//         uiState: { selectedElementId: el.id },
//         elements: [el], // Csak ez az elem létezik most
//         zones: window.savedFullUserData.zones // A zónák kellenek a hibamentes futáshoz!
//     };

//     // 4. RAJZOLÁS ÉS MENTÉS INDÍTÁSA
//     if (typeof Celestial !== 'undefined') {
        
//         // A) Resize (Kényszerített 1000px)
//         Celestial.resize({width: 1000}); 
        
//         // B) Apply (Adatok betöltése)
//         Celestial.apply(myCelestialConf);
        
//         // C) Dátum/Hely visszaállítása a motorban
//         if (myCelestialConf.Ido) Celestial.date(new Date(myCelestialConf.Ido));
//         if (myCelestialConf.geopos) Celestial.location(myCelestialConf.geopos);
...
//         // E) Redraw (Ez aszinkron is lehet, ezért várunk picit)
//         Celestial.redraw();

//         // 5. EXPORTÁLÁS (Snapshot)
//         // Callback definiálása: Mi történjen, ha kész a kép?
//         window.onVectorExportFinished = function() {
//             // Jöhet a következő a sorban!
//             processRenderQueue(elementsList, index + 1);
//         };

//         // Pici késleltetés a DOM frissüléshez, majd indítjuk a mentést
//         setTimeout(() => {
//             window.updateActiveMapSnapshot();
//         }, 150);
//     } else {
//         // Ha valami baj van a Celestiallal, lépjünk tovább, ne fagyjunk le
//         processRenderQueue(elementsList, index + 1);
//     }
// }

// // ============================================================
// // --- RENDER QUEUE (A MEMÓRIA SZIVÁRGÁS ELLEN + ADAT PÓTLÁS) ---
// // ============================================================
// function processRenderQueue(elementsList, index) {
//     // 1. KILÉPÉSI FELTÉTEL: Ha végigértünk a listán
//     if (index >= elementsList.length) {
...
//     // Elmentjük a teljes userDatát, ha még nem tettük (a ciklus elején)
//     if (index === 0 && !window.savedFullUserData) {
//         // Mély másolatot készítünk, hogy biztosan ne vesszen el semmi
//         window.savedFullUserData = JSON.parse(JSON.stringify(myCelestialConf.userData));
//     }
...
//     // --- JAVÍTÁS ITT: PÓTOLJUK A HIÁNYZÓ ADATOKAT (Canvas, ZoneFlags) ---
//     myCelestialConf.userData = {
//         uiState: { 
//             selectedElementId: el.id,
//             // Ez kellett a szövegeknek:
//             zoneFlags: window.savedFullUserData.uiState.zoneFlags || {} 
//         },
//         elements: [el], 
//         zones: window.savedFullUserData.zones,
//         // EZ OKOZTA A HIBÁT (layoutDirection itt lakik):
...
//         // Pici késleltetés a DOM frissüléshez
//         setTimeout(() => {
//             window.updateActiveMapSnapshot();
//         }, 150);
//     } else {
//         // Ha valami baj van a Celestiallal, lépjünk tovább
//         processRenderQueue(elementsList, index + 1);
//     }
// }
...
$(document).ready(function() {
    ...
    //         // --- JAVÍTÁS: KÉNYSZERÍTETT MÉRETEZÉS ÉS FRISSÍTÉS ---
    //         // Több lépcsős frissítés, hogy biztosan elkapjuk a böngésző renderelését
            
    //         // 1. Azonnali méretezés
    //         if(window.updateCanvasSize) window.updateCanvasSize();

    //         // 2. Késleltetett kirajzolás (ha a DOM lassan ébredne)
    //         setTimeout(() => {
    //             // Biztosítjuk, hogy legyen kijelölt elem
    //             if (!myCelestialConf.userData.uiState.selectedElementId) {
    //                 const elements = myCelestialConf.userData.elements;
    //                 if (elements && elements.length > 0) {
    //                     myCelestialConf.userData.uiState.selectedElementId = elements[0].id;
    //                 }
    //             }
                
    //             // Kényszerített újrarajzolás a helyes méretekkel
    //             if (typeof Celestial !== 'undefined') {
    //                 // Ha négyzet volt, ez helyreteszi:
    //                 Celestial.resize({width: 1000}); 
    //                 Celestial.redraw();
    //             }
    ...
    //                 if(window.refreshMapTransform) window.refreshMapTransform();
    //                 if(window.updateElementSelectorUI) window.updateElementSelectorUI();
    //                 // Alapértelmezett fül megnyitása a jobb oldalon
    //                 $("#tabs_r").tabs("option", "active", 0);
    //             });
    //         }, 100); // 100ms várakozás a biztonság kedvéért

    //     } else {
    //         // Kilépés a tervezőből
    //         document.getElementById('full-screen-designer').style.display = 'none';
    //         document.getElementById('main-layout').style.display = 'flex';
            
    //         // Visszaállítás a szerkesztő nézet méretére (hogy ott se legyen torz)
    //         if (typeof getOptimalMapSize === 'function') {
    //             Celestial.resize({width: getOptimalMapSize()});
    //         }
    //     }
    // });
    // // --- TAB VÁLTÁS KEZELÉSE (DESIGNER NÉZET) ---
    // $("#tabs").on("tabsactivate", function(event, ui) {
    //     if (ui.newPanel[0].id === 'fragment-6') { 
    //         // Megjelenítés
    //         document.getElementById('full-screen-designer').style.display = 'block';
    ...
    //         // --- JAVÍTÁS: INPUT MEZŐK ÉS MÉRET INICIALIZÁLÁSA ---
    //         const widthInput = document.getElementById('canvas-width');
    //         const heightInput = document.getElementById('canvas-height');
            
    //         // Ha üres, töltsük ki alapértelmezett értékkel (A4 méret)
    //         if (!widthInput.value) widthInput.value = 21;
    //         if (!heightInput.value) heightInput.value = 30;

    //         // 1. Azonnali méretezés hívása
    //         if(window.updateCanvasSize) window.updateCanvasSize();

    //         // 2. Késleltetett kirajzolás (hogy biztosan legyen DOM méret)
    //         setTimeout(() => {
    //             // Biztosítjuk, hogy legyen kijelölt elem a logikához
    //             if (myCelestialConf.userData && !myCelestialConf.userData.uiState.selectedElementId) {
    //                 const elements = myCelestialConf.userData.elements;
    //                 if (elements && elements.length > 0) {
    //                     myCelestialConf.userData.uiState.selectedElementId = elements[0].id;
    //                 }
    ...
    //             // Kényszerített újrarajzolás a motorban is
    //             if (typeof Celestial !== 'undefined') {
    //                 Celestial.resize({width: 1000}); 
    //                 Celestial.redraw();
    //             }
    ...
    //                 if(window.updateElementSelectorUI) window.updateElementSelectorUI();
    //                 // Alapértelmezett fül megnyitása a jobb oldalon
    //                 try { $("#tabs_r").tabs("option", "active", 0); } catch(e){}
    //             });
    //         }, 150);
    ...
    //         // --- JAVÍTÁS: HELYES ID-K HASZNÁLATA ---
    //         // Megpróbáljuk megtalálni a mezőket a helyes ID-val
    //         let widthInput = document.getElementById('map-width-cm-input');
    //         let heightInput = document.getElementById('map-height-cm-input');
            
    //         // Ha esetleg mégis a régi ID lenne a HTML-ben, fallback
    //         if (!widthInput) widthInput = document.getElementById('canvas-width');
    //         if (!heightInput) heightInput = document.getElementById('canvas-height');

    //         // Ha üres, töltsük ki alapértelmezett értékkel (A4 méret: 21x30)
    //         if (widthInput && !widthInput.value) widthInput.value = 21;
    //         if (heightInput && !heightInput.value) heightInput.value = 30;

    //         // 1. Azonnali méretezés hívása (hogy a vászon mérete beálljon)
    //         if(window.updateCanvasSize) window.updateCanvasSize();
    ...
    //             // Kényszerített újrarajzolás a motorban is
    //             // Ez tünteti el a "négyzetet" és rajzolja ki a térképet
    //             if (typeof Celestial !== 'undefined') {
    //                 Celestial.resize({width: 1000}); 
    //                 Celestial.redraw();
    //             }
    ...
    $("#tabs").on("tabsactivate", function(event, ui) {
        if (ui.newPanel[0].id === 'fragment-6') { 
            ...
            setTimeout(() => {
                ...
                // 1. Kényszerített újrarajzolás (hogy ne legyen négyzet)
                if (typeof Celestial !== 'undefined') {
                    Celestial.resize({width: 1000}); 
                    Celestial.redraw();
                }

                // 2. HIÁNYZÓ SVG-K GENERÁLÁSA (Ha új térkép jött)
                if (typeof window.generateMissingSnapshots === 'function') {
                    window.generateMissingSnapshots();
                } else {
                    // Ha nincs külön függvény, sima refresh
                    if(window.refreshMapTransform) window.refreshMapTransform();
                }
    ...
    if(window.initThemeSelectors) window.initThemeSelectors();
});
