...
Celestial.display = function(config) {
  ...
  function redraw() {  
    ...
    //     // --- ÚJ LOGIKA: KIEMELÉS ---
    //     // Ha van beállított highlightID és ez a csillagkép az...
    //     if (Celestial.highlightID && d.id === Celestial.highlightID) {
    //         // ...akkor egyedi, kiemelt stílust használunk:
    //         var highlightStyle = {
    //             stroke: "#ffcc00",  // Élénk sárga/arany szín (vagy amit szeretnél)
    //             width: 3,           // Vastagabb vonal
    //             opacity: 1,         // Teljesen látható
    //             dash: []            // Nincs szaggatás (folyamatos vonal)
    //         };
    //         setStyle(highlightStyle); // Ezt alkalmazzuk
            
    //         // Opcionális: A Context-et is felülírhatjuk, hogy biztosan a legfelső réteg legyen
    //         // (Canvas-nál a sorrend számít, de itt a redraw sorrendje fix)
    //     } else {
    //         // ...egyébként a szokásos stílust használjuk (rank alapján)
    //         setStyleA(d.properties.rank, cfg.constellations.lineStyle); 
    //     }
...
function form(cfg) {
  ...
  // // --- ÚJ FÜGGVÉNY: CSAK KIEMELÉS (ZOOM NÉLKÜL) ---
  // Celestial.highlight = function(id) {
  //     if (id === "---" || !id) {
  //         Celestial.highlightID = null;
  //     } else {
  //         Celestial.highlightID = id;
  //     }
  //     Celestial.redraw(); // Csak újrarajzolunk, nem animálunk/mozgunk
  // };
...
//       document.getElementById("text-overlay-svg").outerHTML = svg.node().outerHTML;
...
// function copySVG(fname) {// Ideiglenes tároló
//   var doc = d3.select("body").append("div").attr("id", "d3-celestial-svg").attr("style", "display: none"),
//       svg = d3.select("#d3-celestial-svg").append("svg"),
//       m = Celestial.metrics(),
//       cfg = settings.set(),
//       // ... (a többi változó deklaráció marad, ahogy volt) ...
//       proj = projections[cfg.projection],
//       rotation = getAngles(cfg.center),
//       center = [-rotation[0], -rotation[1]],
//       scale0 = proj.scale * m.width/1024,
...
//   svg.selectAll("*").remove();
//   var defs = svg.append("defs");
//   console.log("copySVG m.width", m.width);
//   console.log("copySVG m.height", m.height);
//   // FONTOS: Nem állítunk fix pixeles width/height-et, csak viewBox-ot!
//   svg.attr("viewBox", "0 0 " + m.width + " " + m.height);
//   svg.attr("preserveAspectRatio", "xMidYMid meet");
//   svg.attr("style", "overflow: visible; display: block;"); // Ez engedi, hogy kilógjon a levágás nélkül
...
//   // Az SVG méretét beállítjuk
//   svg.attr("width", m.width).attr("height", m.height);
//   svg.attr("viewBox", "0 0 " + m.width + " " + m.height);
//   // Fontos: overflow visible, hogy ne vágja le, ha skálázzuk
//   svg.attr("style", "overflow: visible;"); 
// // A szükséges Group nevek és style objektumok inicializálása (MÁSOLANDÓ az eredetiből)
//   var groupNames = ['background', 'milkyWay', 'milkyWayBg', 'gridLines', 'constBoundaries', 
//                     'planesequatorial', 'planesecliptic', 'planesgalactic', 'planessupergalactic',
//                     'constLines', 'mapBorder','stars', 'dsos', 'planets', 'gridvaluesLon', 'gridvaluesLat', 
//                     'constNames', 'starDesignations', 'starNames', 'dsoNames', 'planetNames', 'horizon', 'daylight'],
...
//     q.defer(function(callback) { 
//       // d3.json(path + "mw.json", function(error, json) {
//       d3.json("../../data/mw.json", function(error, json) {
//         if (error) callback(error);
//         var mw = getData(json, cfg.transform);
//         var mw_back = getMwbackground(mw);
...
//     q.defer(function(callback) { 
//       // d3.json(path + filename("constellations", "lines"), function(error, json) {
//       d3.json("../../data/" + filename("constellations", "lines"), function(error, json) {
//         if (error) callback(error);
...
//     q.defer(function(callback) { 
//       // d3.json(path +  cfg.stars.data, function(error, json) {
//       d3.json("../../data/" +  cfg.stars.data, function(error, json) {
//         if (error) callback(error);
...
//   // Helper functions
//   // Helper functions (svgStyle, stb...) legyenek itt!
//   function clip(coords) { return proj.clip && d3.geo.distance(center, coords) > halfπ ? 0 : 1; }
//   function point(coords) { return "translate(" + projection(coords) + ")"; }
...
//     return what + sub + cult + ext;
//   }
// function svgStyle(s) { var res = {}; res.fill = s.fill || "none"; res["fill-opacity"] = s.opacity !== null ? s.opacity : 1; res.stroke = s.stroke || "none"; res["stroke-width"] = s.width !== null ? s.width : 0; res["stroke-opacity"] = s.opacity !== null ? s.opacity : 1; if (s.dash) res["stroke-dasharray"] = s.dash.join(" "); else res["stroke-dasharray"] = "none"; res.font = s.font || null; return res; }
...
// function svgAlign(s) { if (!s) return "start"; if (s === "center") return "middle"; if (s === "right") return "end"; return "start"; }
...
//     return res;
//   }
// function createStyles() { var res = ""; for (var key in styles) { if (styles[key]) res += " ." + key + stringifyStyle(styles[key]); } return "/*\u003c![CDATA[*/\n" + res + "\n/*]]\u003e*/"; }
// function stringifyStyle(s) { var res = " {"; for (var key in s) { if (s[key]) res += key + ":" + s[key] + "; "; } return res + "} "; }
...
//   // });
//   // q.await(function(error) {
//   //   console.log("Celestial SVG generation finished.");
//   //   if (error) throw error;
    
//   //   // Stílusok hozzáadása
//   //   defs.append("style").attr("type", "text\/css").text(createStyles());

//   //   // 1. Megkeressük a Tervező nézet rétegét
//   //   var mapLayer = document.getElementById("celestial-map-layer");
    
//   //   if (mapLayer) {
//   //       // 2. Kitöröljük az előző térképet
//   //       mapLayer.innerHTML = '';
        
//   //       // 3. Kivesszük a generált SVG node-ot
//   //       var generatedSvgNode = svg.node();
        
//   //       // 4. Beállítjuk az ID-t, hogy később hivatkozhassunk rá (igazításnál)
//   //       generatedSvgNode.setAttribute("id", "generated-celestial-map");
        
//   //       // 5. Mivel <svg>-t rakunk <g>-be, be kell állítanunk a méreteket, hogy látszódjon
//   //       //    Kezdetben felvesszük a szülő szélességét, de az alignCelestialMap majd korrigálja
//   //       generatedSvgNode.setAttribute("width", m.width);
//   //       generatedSvgNode.setAttribute("height", m.height);
//   //       generatedSvgNode.setAttribute("x", 0);
//   //       generatedSvgNode.setAttribute("y", 0);
        
//   //       // 6. Hozzáadjuk a réteghez
//   //       mapLayer.appendChild(generatedSvgNode);
        
//   //       console.log("Celestial SVG injected into Designer layer.");
        
//   //       // 7. Triggerelünk egy eseményt, vagy hívunk egy globális függvényt, hogy igazítsa középre
//   //       if (typeof window.alignCelestialMap === 'function') {
...
//   //   } else {
//   //       console.error("Nem található a #celestial-map-layer a DOM-ban.");
//   //   }

//   //   // Takarítás: eltávolítjuk a rejtett generáló div-et
//   //   d3.select("#d3-celestial-svg").remove();
//   // });
// // q.await(function(error) {
// //   console.log("q.await");
// //     if (error) throw error;
// //   console.log("q.await no error");
    
// //     // Stílusok generálása
// //     defs.append("style").attr("type", "text\/css").text(createStyles());

// //     // DOM MANIPULÁCIÓ: Beillesztés a Tervezőbe
// //     var targetLayer = document.getElementById("celestial-map-layer");
...
// //         // ID beállítása a hivatkozáshoz
// //         generatedNode.setAttribute("id", "generated-celestial-map");
// //         // Méretek beállítása attribútumként (referenciának), de overflow visible marad
// //         console.log("q.await m", m);
// //         console.log("q.await m.width", m.width);
// //         console.log("q.await m.height", m.height);
// //         generatedNode.setAttribute("width", m.width);
// //         generatedNode.setAttribute("height", m.height);
        
// //         targetLayer.appendChild(generatedNode);
        
// //         console.log("SVG bemásolva. Igazítás indítása...");
        
// //         // Azonnali igazítás hívása
// //         if (typeof window.alignCelestialMap === 'function') {
// //              // setTimeout(() => window.alignCelestialMap('center'), 100);
// //              // setTimeout(() => window.alignCelestialMap(myCelestialConf.alignCelestialMap), 100);
// //              setTimeout(() => window.updateCanvasSize(), 100);
// //         }
// //     }
