...
var Celestial = {
  ...
  container: null,
  data: [],
  // highlightID: null // --- ÚJ: Itt tároljuk a kiemelt csillagkép ID-ját ---,
  highlightList: {} // --- MÓDOSÍTVA: Objektum a többes kiemeléshez { id: {style} } ---
};
  ...
  function redraw() {  
    ...
    if (cfg.constellations.lines) { 
      container.selectAll(parentElement + " .constline").each(function(d) { 
        ...
        // --- ÚJ LOGIKA: TÖBBES EGYEDI KIEMELÉS ---
        // Megnézzük, hogy az aktuális csillagkép (d.id) benne van-e a listában
        if (Celestial.highlightList && Celestial.highlightList[d.id]) {
            // Ha benne van, lekérjük a hozzá mentett stílust
            var customStyle = Celestial.highlightList[d.id];
            
            // Stílus objektum összeállítása a setStyle-hoz
            var highlightStyle = {
                stroke: customStyle.stroke || "#ffcc00",
                width: customStyle.width || 3,
                opacity: customStyle.opacity || 1,
                dash: customStyle.dash || [] // Üres tömb = folyamatos vonal
            };
            
            setStyle(highlightStyle); // Alkalmazzuk az egyedi stílust
        } else {
            // ...egyébként a globális beállítást használjuk
...
var settings = { 
  ...
  lines: {
    graticule: { show: true, stroke: "#cccccc", width: 0.6, opacity: 0.8,  dash: [],     // Show graticule lines 
      ...
      lat: {pos: [], fill: "#FF0000", font: "10px 'Lucida Sans Unicode', Helvetica, Arial, sans-serif"}},
...
*/
var projections = {
  "customHeart": {n: "customHeart", arg: Math.PI / 2.5, scale: 210, ratio: 0.88}, // A te jól bevált arányod
...
function form(cfg) {
  ...
  col.append("input").attr("type", "button").attr("id", "copy-svg").attr("value", "SVG File").on("click", function() {
    copySVG('getFilename(".svg")'); 
    return false;
  });
  ...
  // --- ÚJ FÜGGVÉNY: TÖBBES KIEMELÉS KEZELÉSE ---
  // id: csillagkép kódja (pl. "Ori")
  // style: objektum { stroke, width, opacity, dash: [vonal, szünet] }
  // Ha style null, akkor töröljük a kiemelést.
  // Ha style undefined (nincs megadva), akkor ALAPÉRTELMEZETT stílust használunk.
  Celestial.highlight = function(id, style) {
      if (!id || id === "---") return;

      if (style === null) {
          // Törlés
          delete Celestial.highlightList[id];
      } else {
          // Ha nem kaptunk stílust, legyen alapértelmezett (Sárga, vastag)
          if (!style) {
              style = {
                  stroke: "#ffcc00",
                  width: 3,
          ...
          Celestial.highlightList[id] = {
              stroke: style.stroke || "#ffcc00",
              width: style.width || 3,
              opacity: style.opacity || 1,
              dash: style.dash || []
          };
      }
      Celestial.redraw(); // Újrarajzolás a változásokkal
  };
  
  // Segédfüggvény az összes kiemelés törléséhez (pl. reset gombhoz)
  Celestial.clearHighlights = function() {
...
// Export lock to prevent simultaneous exports
var isExportInProgress = false;

function exportSVG(fname) {
  // console.log("exportSVG - JAVÍTOTT (Háttér visszaállítva, Inkscape fix)");

  if (isExportInProgress) {
      console.warn("Export folyamatban, duplikált hívás eldobva.");
      return;
  }
  ...
  var doc = d3.select("body").append("div").attr("id", "d3-celestial-svg").attr("style", "display: none"),
      ...
      TARGET_WIDTH = 3500, // Nagy felbontás
      mult = TARGET_WIDTH / rawM.width,
      m = {
          width: TARGET_WIDTH,
          height: rawM.height * mult,
          margin: [rawM.margin[0] * mult, rawM.margin[1] * mult],
          scale: rawM.scale * mult 
      },
      
      // FIX: Don't reset cfg! Use the existing config that was set by Celestial.apply()
      // cfg = settings.set(),  // REMOVED - this was resetting to defaults!
      path = cfg.datapath,
      proj = projections[cfg.projection],
      rotation = getAngles(cfg.center),
      center = [-rotation[0], -rotation[1]],
      scale0 = proj.scale * m.width/1024,
      originalTrans = (typeof mapProjection !== 'undefined' && mapProjection) ? mapProjection.translate() : [rawM.width/2, rawM.height/2],
  ...
  // Skálázási arány (Referencia: 1440px széles képernyő)
  // Ez biztosítja, hogy a vonalak és csillagok arányosak maradjanak nagyban is
  ...
  // SVG ELEM BEÁLLÍTÁSA (INKSCAPE NÉVTÉR NÉLKÜL!)
  svg.attr("width", m.width)
  ...
  for (var i=0; i<groupNames.length; i++) {
    // Sima csoportok, Inkscape attribútumok nélkül
    groups[groupNames[i]] = svg.append('g').attr("id", groupNames[i]);
    styles[groupNames[i]] = {};
  }

  // --- SEGÉDFÜGGVÉNYEK (Hogy ne legyen ReferenceError) ---
  function filename(what, sub, ext) {
    var cult = (has(formats[what], culture)) ? "." + culture : "";
    ext = ext ? "." + ext : ".json";
    sub = sub ? "." + sub : "";
  ...
  function constName(d) { return d.properties[cfg.constellations.namesType]; }
  function starDesignation(id) { if (!has(starnames, id)) return ""; return starnames[id][cfg.stars.designationType]; }
  function starPropername(id) { var lang = cfg.stars.propernameType; if (!has(starnames, id)) return ""; return has(starnames[id], lang) ? starnames[id][lang] : starnames[id].name; }
  function dsoName(d) { var lang = cfg.dsos.namesType, id = d.id; if (lang === "desig" || !has(dsonames, id)) return d.properties.desig; return has(dsonames[id], lang) ? dsonames[id][lang] : d.properties.desig; }
  ...
  function dsoSymbol(p) {
    var size = dsoSize(p.mag, p.dim) || 9 * exportScaleRatio, type = dsoShape(p.type);
    if (d3.svg.symbolTypes.indexOf(type) !== -1) return d3.svg.symbol().type(type).size(size)();
    else return d3.svg.customSymbol().type(type).size(size)();
  }
  function dsoShape(type) {
    if (!type || !has(cfg.dsos.symbols, type)) return "circle"; 
    else return cfg.dsos.symbols[type].shape; 
  ...
  function createEntry(o) {
    ...
    if (res.id === "lun") { res.properties.age = o.ephemeris.age; res.properties.phase = o.ephemeris.phase; }
  ...
  function createStyles() {
    var res = "";
    for (var key in styles) { if (!has(styles, key)) continue; res += " ." + key + stringifyStyle(styles[key]); }
    return "/*\u003c![CDATA[*/\n" + res + "\n/*]]\u003e*/";
  }
  function stringifyStyle(s) {
    var res = " {";
    for (var key in s) { if (!has(s, key)) continue; res += key + ":" + s[key] + "; "; }
    return res + "} ";
  }
  function getGridValues(type, loc) {
     // Egyszerűsített grid values generátor exportra, ha szükséges
     // A Celestial objektumból is elérhető lenne, de itt biztonságosabb
     return []; // Grid feliratok nélkül az export tisztább, de ha kell, ide másolható
  }
  ...
  // 1. TÉRKÉP ALAP KÖR (Background)
  groups.background.append("path").datum(circle).attr("class", "background").attr("d", map);
  // VISSZAÁLLÍTVA A KONFIGURÁCIÓRA
  console.log("🎨 DEBUG exportSVG cfg.background:", JSON.stringify(cfg.background));
  styles.background.fill = cfg.background.fill;
  styles.background.stroke = cfg.background.stroke;
  styles.background["stroke-width"] = cfg.background.width * exportScaleRatio;
  styles.background["fill-opacity"] = cfg.background.opacity;
  ...
  if (cfg.mw.show) {
    q.defer(function(callback) { 
      d3.json(path + "mw.json", function(error, json) {
        ...
        // VISSZAÁLLÍTVA A KONFIGURÁCIÓRA (De csak ha nincs nagy átlátszóság)
        if (!has(cfg.background, "opacity") || cfg.background.opacity > 0.95) {
             ...
             styles.milkyWayBg = {"fill": cfg.background.fill, "fill-opacity": cfg.background.opacity };
        }
  ...
  if (cfg.constellations.bounds) { 
    q.defer(function(callback) { 
      d3.json(path + filename("constellations", "borders"), function(error, json) {
        ...
        if (Celestial.constellation) { var re = new RegExp("\\b" + Celestial.constellation + "\\b"); }
        groups.constBoundaries.selectAll(".bounds").data(conb.features).enter().append("path")
        ...
        styles.constBoundariesSel = {"fill": "none", "stroke": cfg.constellations.boundStyle.stroke, "stroke-width": cfg.constellations.boundStyle.width * 1.5 * exportScaleRatio, "stroke-opacity": 1, "stroke-dasharray": "none"};
        callback(null);
      });
    });
  }
  ...
  if (cfg.constellations.lines) { 
    q.defer(function(callback) { 
      d3.json(path + filename("constellations", "lines"), function(error, json) {
        if (error) callback(error);
        var conl = getData(json, cfg.transform);
        groups.constLines.selectAll(".lines").data(conl.features).enter().append("path")
         .attr("class", function(d) { 
             if (Celestial.highlightList && Celestial.highlightList[d.id]) return "constLines_highlight_" + d.id;
        ...
        styles.constLines1 = {"fill": "none", "stroke": cfg.constellations.lineStyle.stroke[0], "stroke-width": cfg.constellations.lineStyle.width[0] * exportScaleRatio, "stroke-opacity": cfg.constellations.lineStyle.opacity[0], "stroke-dasharray": dasharray};
        styles.constLines2 = {"fill": "none", "stroke": cfg.constellations.lineStyle.stroke[1], "stroke-width": cfg.constellations.lineStyle.width[1] * exportScaleRatio, "stroke-opacity": cfg.constellations.lineStyle.opacity[1], "stroke-dasharray": dasharray};
        styles.constLines3 = {"fill": "none", "stroke": cfg.constellations.lineStyle.stroke[2], "stroke-width": cfg.constellations.lineStyle.width[2] * exportScaleRatio, "stroke-opacity": cfg.constellations.lineStyle.opacity[2], "stroke-dasharray": dasharray};
                              
        if (Celestial.highlightList) {
            for (var hid in Celestial.highlightList) {
                var hStyle = Celestial.highlightList[hid];
                var hDash = (hStyle.dash && hStyle.dash.length > 0) ? hStyle.dash.map(d => d * exportScaleRatio).join(" ") : "none";
                styles["constLines_highlight_" + hid] = {
                    "fill": "none", "stroke": hStyle.stroke || "#ffcc00", "stroke-width": (hStyle.width || 3) * exportScaleRatio, "stroke-opacity": hStyle.opacity || 1, "stroke-dasharray": hDash
                };
            }
        }
        callback(null);
    ...
    q.defer(function(callback) { 
      d3.json(path + filename("constellations"), function(error, json) {
        ...
        groups.constNames.selectAll(".constnames")
         .data(conn.features.filter( function(d) { return clip(d.geometry.coordinates) === 1; }))
         .enter().append("text")
        ...
        styles.constNames1 = {"fill": cfg.constellations.nameStyle.fill[0], "fill-opacity": cfg.constellations.nameStyle.opacity[0], "font": scaleExportFont(cfg.constellations.nameStyle.font[0]), "text-anchor": svgAlign(cfg.constellations.nameStyle.align)};
        styles.constNames2 = {"fill": cfg.constellations.nameStyle.fill[1], "fill-opacity": cfg.constellations.nameStyle.opacity[1], "font": scaleExportFont(cfg.constellations.nameStyle.font[1]), "text-anchor": svgAlign(cfg.constellations.nameStyle.align)};
        styles.constNames3 = {"fill": cfg.constellations.nameStyle.fill[2], "fill-opacity": cfg.constellations.nameStyle.opacity[2], "font": scaleExportFont(cfg.constellations.nameStyle.font[2]), "text-anchor": svgAlign(cfg.constellations.nameStyle.align)};
        callback(null);
      });
    });
  }
    ...
    q.defer(function(callback) { 
      d3.json(path +  cfg.stars.data, function(error, json) {
        ...
        groups.stars.selectAll(".stars")
          .data(cons.features.filter( function(d) { return d.properties.mag <= cfg.stars.limit; }))
          .enter().append("path")
        ...
        if (cfg.stars.designation) { 
          groups.starDesignations.selectAll(".stardesigs")
            .data(cons.features.filter( function(d) { return d.properties.mag <= cfg.stars.designationLimit*adapt && clip(d.geometry.coordinates) === 1; }))
            .enter().append("text").attr("transform", function(d) { return point(d.geometry.coordinates); })
            .text( function(d) { return starDesignation(d.id); }).attr({dy: ".85em", dx: ".35em", class: "starDesignations"});
        ...
        if (cfg.stars.propername) { 
          groups.starNames.selectAll(".starnames")
            .data(cons.features.filter( function(d) { return d.properties.mag <= cfg.stars.propernameLimit*adapt && clip(d.geometry.coordinates) === 1; }))
            .enter().append("text").attr("transform", function(d) { return point(d.geometry.coordinates); })
            .text( function(d) { return starPropername(d.id); }).attr({dy: "-.5em", dx: "-.35em", class: "starNames"});
    ...
    q.defer(function(callback) { 
      d3.json(path +  cfg.dsos.data, function(error, json) {
        ...
        groups.dsos.selectAll(".dsos").data(cond.features.filter( function(d) {
          ...
          })).enter().append("path").attr("class", function(d) { return "dsos" + d.properties.type; }).attr("transform", function(d) { return point(d.geometry.coordinates); }).attr("d", function(d) { return dsoSymbol(d.properties); });
        ...
        for (key in cfg.dsos.symbols) {
          ...
          if (has(cfg.dsos.symbols[key], "stroke")) {
            styles[id].fill = "none"; styles[id].stroke = cfg.dsos.colors ? cfg.dsos.symbols[key].stroke : cfg.dsos.style.stroke; styles[id]["stroke-width"] = (cfg.dsos.colors ? cfg.dsos.symbols[key].width : cfg.dsos.style.width) * exportScaleRatio;
          } else {
            styles[id].stroke = "none"; styles[id].fill = cfg.dsos.colors ? cfg.dsos.symbols[key].fill : cfg.dsos.style.fill;
          }
        }
        if (cfg.dsos.names) { 
          groups.dsoNames.selectAll(".dsonames").data(cond.features.filter( function(d) {
            ...
            })).enter().append("text").attr("class", function(d) { return "dsoNames " + d.properties.type; }).attr("transform", function(d) { return point(d.geometry.coordinates); }).text( function(d) { return dsoName(d); }).attr({dy: "-.5em", dx: ".35em"});
          styles.dsoNames = {"fill-opacity": cfg.dsos.style.opacity, "font": scaleExportFont(cfg.dsos.nameStyle.font), "text-anchor": svgAlign(cfg.dsos.nameStyle.align)};
          for (key in cfg.dsos.symbols) { if (!has(cfg.dsos.symbols, key)) continue; styles[key] = {"fill": cfg.dsos.colors ? cfg.dsos.symbols[key].fill : cfg.dsos.style.fill }; }
        }
        callback(null);
      });
    });
  ...
  if ((cfg.location || cfg.formFields.location) && cfg.planets.show && Celestial.origin) {
    q.defer(function(callback) {
      var dt = Celestial.date(), o = Celestial.origin(dt).spherical(), jp = {type: "FeatureCollection", features: []}, jlun = {type: "FeatureCollection", features: []};
      Celestial.container.selectAll(".planet").each(function(d) {
        var id = d.id(), r = 12, p = d(dt).equatorial(o);
        p.ephemeris.pos = transformDeg(p.ephemeris.pos, euler[cfg.transform]); 
      ...
      if (cfg.planets.symbolType === "disk") {
        groups.planets.selectAll(".planets").data(jp.features).enter().append("path").attr("transform", function(d) { return point(d.geometry.coordinates); }).attr("d", function(d) { 
        ...
        groups.planets.selectAll(".planets").data(jp.features).enter().append("text").attr("transform", function(d) { return point(d.geometry.coordinates); }).text( function(d) { return d.properties.symbol; }).attr("class", function(d) { return "planets " + d.id; }).attr({dy: ".35em"});
      }
      if (jlun.features.length > 0) {
        if (cfg.planets.symbolType === "letter") {
          groups.planets.selectAll(".moon").data(jlun.features).enter().append("text").attr("transform", function(d) { return point(d.geometry.coordinates); }).text( function(d) { return d.properties.symbol; }).attr("class", function(d) { return "planets " + d.id; }).attr({dy: ".35em"});
        } else {
          var rl = has(cfg.planets.symbols.lun, "size") ? (cfg.planets.symbols.lun.size - 1) * adapt * exportScaleRatio : 11 * adapt * exportScaleRatio; 
          groups.planets.selectAll(".dmoon").data(jlun.features).enter().append("path").attr("class", "darkluna" ).attr("transform", function(d) { return point(d.geometry.coordinates); }).attr("d", function(d) { return d3.svg.symbol().type("circle").size(rl*rl)(); });
          groups.planets.selectAll(".moon").data(jlun.features).enter().append("path").attr("class", "planets " + d.id).attr("transform", function(d) { return point(d.geometry.coordinates); }).attr("d", function(d) { return moonSymbol(d.properties, rl); });
        }
      } 
      styles.planets = svgTextStyle(cfg.planets.symbolStyle);
      styles.planets.font = scaleExportFont(cfg.planets.symbolStyle.font); // Font skálázás
      styles.darkluna = {"fill": "#557"};
      for (key in cfg.planets.symbols) { if (!has(cfg.planets.symbols, key)) continue; styles[key] = {"fill": cfg.planets.symbols[key].fill}; }
      if (cfg.planets.names) {
        groups.planetNames.selectAll(".planetnames").data(jp.features).enter().append("text").attr("transform", function(d) { return point(d.geometry.coordinates); }).text( function(d) { return d.properties.name; }).attr({dy: ".85em", dx: "-.35em"}).attr("class", function(d) { return "planetNames " + d.id; });
        if (jlun.features.length > 0) {
          groups.planetNames.selectAll(".moonname").data(jlun.features).enter().append("text").attr("transform", function(d) { return point(d.geometry.coordinates); }).text( function(d) { return d.properties.name; }).attr({dy: ".85em", dx: "-.35em"}).attr("class", function(d) { return "planetNames " + d.id; });
        }
      }
  ...
  q.await(function(error) {
    if (error) {
        console.error("Celestial Export Hiba:", error);
        d3.select("#d3-celestial-svg").remove();
        isExportInProgress = false;
        return;
    }

    var svgSelect = d3.select("#d3-celestial-svg svg");
    if (svgSelect.empty() || !svgSelect.node()) {
        d3.select("#d3-celestial-svg").remove();
        isExportInProgress = false;
        return;
    }

    var svgNode = svgSelect.node();
    svgSelect.attr("viewBox", " 0 0 " + (m.width) + " " + (m.height));

    try {
        var cssStyles = createStyles();
        defs.append("style").attr("type", "text\/css").text(cssStyles);
    } catch(e) {}

    if (fname) {
      try {
          var blob = new Blob([svgNode.outerHTML], {type:"image/svg+xml;charset=utf-8"});
          var a = d3.select("body").append("a").node(); 
      ...
      if (typeof window.handleVectorExport === 'function') {
          window.handleVectorExport(svgNode.outerHTML); 
      }
    }
...
Celestial.copySVG = function(callback) {
  if (!callback) return;
  exportCallback = callback;
  copySVG();
};
